...
$OSCOM_Language->load_definitions('password_reset');
$error = false;
if (!isset($_GET['account']) || !isset($_GET['key'])) {
    $error = true;
    $message_stack->add_session('password_forgotten', OSCOM::get_def('text_no_reset_link_found'));
}
if ($error == false) {
    $email_address = HTML::sanitize($_GET['account']);
    $password_key = HTML::sanitize($_GET['key']);
    if (strlen((string) $email_address) < ENTRY_EMAIL_ADDRESS_MIN_LENGTH || Is::email($email_address) == false) {
        $error = true;
        $message_stack->add_session('password_forgotten', OSCOM::get_def('text_no_email_address_found'));
    } elseif (strlen((string) $password_key) != 40) {
        $error = true;
        $message_stack->add_session('password_forgotten', OSCOM::get_def('text_no_reset_link_found'));
    } else {
...
if (isset($_GET['action']) && $_GET['action'] == 'process' && isset($_POST['formid']) && $_POST['formid'] == $_SESSION['sessiontoken']) {
    ...
    if (strlen((string) $password_new) < ENTRY_PASSWORD_MIN_LENGTH) {
        $error = true;
        $message_stack->add('password_reset', OSCOM::get_def('entry_password_new_error', ['min_length' => ENTRY_PASSWORD_MIN_LENGTH]));
    } elseif ($password_new != $password_confirmation) {
        $error = true;
        $message_stack->add('password_reset', OSCOM::get_def('entry_password_new_error_not_matching'));
    }
    if ($error == false) {
        $OSCOM_Db->save('customers', ['customers_password' => Hash::encrypt($password_new)], ['customers_id' => $Qcheck->value_int('customers_id')]);
        $OSCOM_Db->save('customers_info', ['customers_info_date_account_last_modified' => 'now()', 'password_reset_key' => 'null', 'password_reset_date' => 'null'], ['customers_info_id' => $Qcheck->value_int('customers_id')]);
        $message_stack->add_session('login', OSCOM::get_def('success_password_reset'), 'success');
...
?></label>
      <div class="col-sm-9">
        <?php 
echo HTML::password_field('password', null, 'required aria-required="true" autofocus="autofocus" id="inputPassword" autocomplete="new-password" placeholder="' . OSCOM::get_def('entry_password_text') . '"');
?>
        <?php 
...
?></label>
      <div class="col-sm-9">
        <?php 
echo HTML::password_field('confirmation', null, 'required aria-required="true" id="inputConfirm" autocomplete="new-password" placeholder="' . OSCOM::get_def('entry_password_confirmation_text') . '"');
?>
        <?php 
echo OSCOM::get_def('form_required_input');
?>
