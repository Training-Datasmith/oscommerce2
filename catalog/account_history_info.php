...
$Qcheck->bind_int(':orders_id', $_GET['order_id']);
$Qcheck->bind_int(':language_id', $OSCOM_Language->get_id());
$Qcheck->execute();
if ($Qcheck->fetch() === false || $Qcheck->value_int('customers_id') != $_SESSION['customer_id']) {
    OSCOM::redirect('account_history.php');
}
$OSCOM_Language->load_definitions('account_history_info');
...
$breadcrumb->add(OSCOM::get_def('navbar_title_3', ['order_id' => $_GET['order_id']]), OSCOM::link('account_history_info.php', 'order_id=' . $_GET['order_id']));
...
echo OSCOM::get_def('heading_order_number', ['order_id' => $_GET['order_id']]) . ' <span class="badge pull-right">' . $order->info['orders_status'] . '</span>';
?></strong></div>
...
if (sizeof($order->info['tax_groups']) > 1) {
    ...
    echo OSCOM::get_def('heading_products');
    ?></strong></td>
            <td align="right"><strong><?php 
    echo OSCOM::get_def('heading_tax');
    ?></strong></td>
            <td align="right"><strong><?php 
    echo OSCOM::get_def('heading_total');
    ?></strong></td>
          </tr>
<?php 
} else {
...
echo OSCOM::get_def('heading_order_total') . ' ' . $order->info['total'];
?></span><?php 
echo OSCOM::get_def('heading_order_date') . ' ' . DateTime::to_long($order->info['date_purchased']);
?>
      </div>
    </div>
  </div>
...
if ($order->delivery != false) {
    ...
    echo '<strong>' . OSCOM::get_def('heading_delivery_address') . '</strong>';
    ?></div>
          <div class="panel-body">
            <?php 
...
?>
    <div class="col-sm-4">
      <div class="panel panel-warning">
        <div class="panel-heading"><?php 
echo '<strong>' . OSCOM::get_def('heading_billing_address') . '</strong>';
?></div>
        <div class="panel-body">
          <?php 
...
if ($order->info['shipping_method']) {
    ?>
        <div class="panel panel-info">
          <div class="panel-heading"><?php 
    echo '<strong>' . OSCOM::get_def('heading_shipping_method') . '</strong>';
    ?></div>
          <div class="panel-body">
            <?php 
...
?>
      <div class="panel panel-warning">
        <div class="panel-heading"><?php 
echo '<strong>' . OSCOM::get_def('heading_payment_method') . '</strong>';
?></div>
        <div class="panel-body">
          <?php 
...
?>
  ...
  <h2><?php 
echo OSCOM::get_def('heading_order_history');
?></h2>
...
$Qstatuses->bind_int(':orders_id', $_GET['order_id']);
...
while ($Qstatuses->fetch()) {
    ...
    echo '      <p class="pull-right"><small class="text-muted"><i class="fa fa-clock-o"></i> ' . DateTime::to_short($Qstatuses->value('date_added')) . '</small></p><h2 class="timeline-title">' . $Qstatuses->value('orders_status_name') . '</h2>';
    echo '    </div>';
    echo '    <div class="timeline-body">';
    echo '      <p>' . (tep_not_null($Qstatuses->value('comments')) ? '<blockquote>' . nl2br((string) $Qstatuses->value_protected('comments')) . '</blockquote>' : '&nbsp;') . '</p>';
    echo '    </div>';
    echo '  </div>';
    echo '</li>';
}
...
?>
  ...
  <div class="buttonSet">
    <?php 
echo HTML::button(OSCOM::get_def('image_button_back'), 'fa fa-angle-left', OSCOM::link('account_history.php', tep_get_all_get_params(['order_id'])));
?>
  </div>
</div>
