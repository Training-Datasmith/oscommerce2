...
$Qdownload->bind_int(':orders_id', $_GET['order']);
$Qdownload->bind_int(':customers_id', $_SESSION['customer_id']);
$Qdownload->bind_int(':orders_products_download_id', $_GET['id']);
$Qdownload->bind_int(':language_id', $OSCOM_Language->get_id());
$Qdownload->execute();
...
$download_timestamp = mktime(23, 59, 59, $dt_month, $dt_day + $Qdownload->value_int('download_maxdays'), $dt_year);
// Die if time expired (maxdays = 0 means no time limit)
if ($Qdownload->value_int('download_maxdays') != 0 && $download_timestamp <= time()) {
    die;
}
// Die if remaining count is <=0
if ($Qdownload->value_int('download_count') <= 0) {
    die;
}
// Die if file is not there
$download_filename = basename($Qdownload->value('orders_products_filename'));
$download_base_dir = realpath(OSCOM::get_config('dir_root') . 'download');
...
$Qupdate->bind_int(':orders_products_download_id', $_GET['id']);
$Qupdate->execute();
...
function tep_random_name(): string
{
    ...
    $dirname = '.';
    $length = floor(Hash::get_random_int(16, 20));
    for ($i = 1; $i <= $length; $i++) {
        $q = floor(Hash::get_random_int(1, 26));
        $dirname .= $letters[$q];
    }
    return $dirname;
}
...
if (DOWNLOAD_BY_REDIRECT == 'true') {
    ...
    tep_unlink_temp_dir(OSCOM::get_config('dir_root') . 'pub/');
    $tempdir = tep_random_name();
    umask(00);
    mkdir(OSCOM::get_config('dir_root') . 'pub/' . $tempdir, 0750);
    symlink($download_filepath, OSCOM::get_config('dir_root', 'Shop') . 'pub/' . $tempdir . '/' . $download_filename);
    if (is_file(OSCOM::get_config('dir_root') . 'pub/' . $tempdir . '/' . $download_filename)) {
