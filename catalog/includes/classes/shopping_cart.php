...
class Shopping_Cart
{
    ...
    public function restore_contents()
    {
        ...
        if (is_array($this->contents)) {
            foreach (array_keys($this->contents) as $products_id) {
                ...
                $Qcheck->bind_value(':products_id', $products_id);
                $Qcheck->execute();
        ...
        while ($Qproducts->fetch()) {
            $this->contents[$Qproducts->value('products_id')] = ['qty' => $Qproducts->value_int('customers_basket_quantity')];
            // attributes
            ...
            $Qattributes->bind_int(':customers_id', $_SESSION['customer_id']);
            $Qattributes->bind_value(':products_id', $Qproducts->value('products_id'));
            $Qattributes->execute();
            while ($Qattributes->fetch()) {
                $this->contents[$Qproducts->value('products_id')]['attributes'][$Qattributes->value_int('products_options_id')] = $Qattributes->value_int('products_options_value_id');
            }
        }
        $this->cleanup();
        ...
        if (is_array($attributes) && !empty($attributes)) {
            foreach ($attributes as $option => $value) {
                if (!is_numeric($option) || !is_numeric($value)) {
                    ...
                    $Qcheck->bind_int(':options_id', $option);
                    $Qcheck->bind_int(':options_values_id', $value);
                    $Qcheck->execute();
                    if ($Qcheck->fetch() === false) {
                        $attributes_pass_check = false;
                        break;
    ...
    public function count_contents(): int|float
    {
        $total_items = 0;
        if (is_array($this->contents)) {
            foreach (array_keys($this->contents) as $products_id) {
    ...
    public function in_cart($products_id): bool
    {
        if (isset($this->contents[$products_id])) {
            return true;
        }
    ...
    public function calculate()
    {
        ...
        foreach (array_keys($this->contents) as $products_id) {
            ...
            $Qproduct->bind_int(':products_id', $products_id);
            $Qproduct->execute();
            if ($Qproduct->fetch() !== false) {
                $prid = $Qproduct->value_int('products_id');
                $products_tax = tep_get_tax_rate($Qproduct->value_int('products_tax_class_id'));
                $products_price = $Qproduct->value_decimal('products_price');
                $products_weight = $Qproduct->value_decimal('products_weight');
                ...
                $Qspecial->bind_int(':products_id', $prid);
                $Qspecial->execute();
                if ($Qspecial->fetch() !== false) {
                    $products_price = $Qspecial->value_decimal('specials_new_products_price');
                }
                ...
                if (isset($this->contents[$products_id]['attributes'])) {
                    foreach ($this->contents[$products_id]['attributes'] as $option => $value) {
                        ...
                        $Qattributes->bind_int(':products_id', $prid);
                        ...
                        if ($Qattributes->fetch() !== false) {
                            if ($Qattributes->value('price_prefix') == '+') {
                                $this->total += $currencies->calculate_price($Qattributes->value_decimal('options_values_price'), $products_tax, $qty);
                            } else {
                                $this->total -= $currencies->calculate_price($Qattributes->value_decimal('options_values_price'), $products_tax, $qty);
                            }
                        }
    ...
    public function attributes_price($products_id): float|int
    {
        $OSCOM_Db = Registry::get('Db');
        $attributes_price = 0;
        if (isset($this->contents[$products_id]['attributes'])) {
            foreach ($this->contents[$products_id]['attributes'] as $option => $value) {
                ...
                if ($Qattributes->fetch() !== false) {
                    if ($Qattributes->value('price_prefix') == '+') {
                        $attributes_price += $Qattributes->value_decimal('options_values_price');
                    } else {
                        $attributes_price -= $Qattributes->value_decimal('options_values_price');
                    }
                }
    ...
    public function get_products(): false|array
    {
        $OSCOM_Db = Registry::get('Db');
        ...
        foreach (array_keys($this->contents) as $products_id) {
            ...
            $Qproducts->bind_int(':products_id', $products_id);
            $Qproducts->bind_int(':language_id', $OSCOM_Language->get_id());
            $Qproducts->execute();
            if ($Qproducts->fetch() !== false) {
                $products_price = $Qproducts->value_decimal('products_price');
                $Qspecial = $OSCOM_Db->prepare('select specials_new_products_price from :table_specials where products_id = :products_id and status = 1');
                $Qspecial->bind_int(':products_id', $Qproducts->value_int('products_id'));
                $Qspecial->execute();
                if ($Qspecial->fetch() !== false) {
                    $products_price = $Qspecial->value_decimal('specials_new_products_price');
                }
                $products_array[] = ['id' => $products_id, 'name' => $Qproducts->value('products_name'), 'model' => $Qproducts->value('products_model'), 'image' => $Qproducts->value('products_image'), 'price' => $products_price, 'quantity' => $this->contents[$products_id]['qty'], 'weight' => $Qproducts->value_decimal('products_weight'), 'final_price' => $products_price + $this->attributes_price($products_id), 'tax_class_id' => $Qproducts->value_int('products_tax_class_id'), 'attributes' => $this->contents[$products_id]['attributes'] ?? ''];
            }
        }
    ...
    public function generate_cart_id($length = 5): string|false
    {
        return Hash::get_random_string($length, 'digits');
    }
    public function get_content_type(): string|false
    {
        $OSCOM_Db = Registry::get('Db');
        $this->content_type = false;
