...
class shipping
{
        ...
        if (defined('MODULE_SHIPPING_INSTALLED') && tep_not_null(MODULE_SHIPPING_INSTALLED)) {
            ...
            if (isset($code) && (in_array($code . '.' . substr((string) $PHP_SELF, strrpos((string) $PHP_SELF, '.') + 1), $this->modules) || in_array($code, $this->modules))) {
                if (str_contains($code, '\\')) {
                    $class = Apps::get_module_class($code, 'Shipping');
                    $include_modules[] = ['class' => $code, 'file' => $class];
                } else {
                    $include_modules[] = ['class' => $code, 'file' => $code . '.' . substr((string) $PHP_SELF, strrpos((string) $PHP_SELF, '.') + 1)];
                }
            } else {
                foreach ($this->modules as $value) {
                    if (str_contains($value, '\\')) {
                        $class = Apps::get_module_class($value, 'Shipping');
            ...
            for ($i = 0, $n = sizeof($include_modules); $i < $n; $i++) {
                if (str_contains($include_modules[$i]['class'], '\\')) {
                    ...
                    $this->lang->load_definitions('modules/shipping/' . pathinfo((string) $include_modules[$i]['file'], PATHINFO_FILENAME));
     ...
     * @return mixed[][]
     */
    public function quote($method = '', $module = ''): array
    {
            ...
            if ($obj->enabled) {
                foreach ($obj->quotes['methods'] as $method) {
                    if (isset($method['cost']) && tep_not_null($method['cost'])) {
                        return ['id' => $obj->quotes['id'] . '_' . $method['id'], 'title' => $obj->quotes['module'] . (isset($method['title']) && !empty($method['title']) ? ' (' . $method['title'] . ')' : ''), 'cost' => $method['cost']];
                    }
                }
            }
        }
                ...
                if ($obj->enabled) {
                    ...
                    for ($i = 0, $n = sizeof($quotes['methods']); $i < $n; $i++) {
                        if (isset($quotes['methods'][$i]['cost']) && tep_not_null($quotes['methods'][$i]['cost'])) {
                            $rates[] = ['id' => $quotes['id'] . '_' . $quotes['methods'][$i]['id'], 'title' => $quotes['module'] . (isset($quotes['methods'][$i]['title']) && !empty($quotes['methods'][$i]['title']) ? ' (' . $quotes['methods'][$i]['title'] . ')' : ''), 'cost' => $quotes['methods'][$i]['cost']];
                        }
                    }
                }
            }
