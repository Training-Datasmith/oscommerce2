...
class payment
{
        ...
        if (defined('MODULE_PAYMENT_INSTALLED') && tep_not_null(MODULE_PAYMENT_INSTALLED)) {
            ...
            if (tep_not_null($module) && (in_array($module . '.' . substr((string) $PHP_SELF, strrpos((string) $PHP_SELF, '.') + 1), $this->modules) || in_array($module, $this->modules))) {
                ...
                if (str_contains($module, '\\')) {
                    $class = Apps::get_module_class($module, 'Payment');
                ...
                foreach ($this->modules as $value) {
                    if (str_contains($value, '\\')) {
                        $class = Apps::get_module_class($value, 'Payment');
            ...
            for ($i = 0, $n = sizeof($include_modules); $i < $n; $i++) {
                if (str_contains($include_modules[$i]['class'], '\\')) {
                    ...
                    $this->lang->load_definitions('modules/payment/' . pathinfo((string) $include_modules[$i]['file'], PATHINFO_FILENAME));
    ...
    public function update_status(): void
    {
        if (is_array($this->modules)) {
            ...
            } else if (is_object($GLOBALS[$this->selected_module])) {
    ...
    public function javascript_validation(): string
    {
        ...
        if (is_array($this->modules)) {
            ...
            $js .= "\n" . '  if (payment_value == null) {' . "\n" . '    error_message = error_message + ' . json_encode(OSCOM::get_def('js_error_no_payment_module_selected') . "\n") . ';' . "\n" . '    error = 1;' . "\n" . '  }' . "\n\n" . '  if (error == 1) {' . "\n" . '    alert(error_message);' . "\n" . '    return false;' . "\n" . '  } else {' . "\n" . '    return true;' . "\n" . '  }' . "\n" . '}' . "\n" . '//--></script>' . "\n";
        }
        return $js;
    }
    /**
    ...
    public function pre_confirmation_check(): void
    {
        if (is_array($this->modules)) {
            ...
            } else if (is_object($GLOBALS[$this->selected_module]) && $GLOBALS[$this->selected_module]->enabled) {
