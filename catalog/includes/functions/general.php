...
function tep_get_products_name($product_id, $language_id = null)
{
    ...
    $Qproduct->bind_int(':products_id', $product_id);
    $Qproduct->bind_int(':language_id', $language_id);
    $Qproduct->execute();
...
function tep_get_products_special_price($product_id)
{
    ...
    if ($Qproduct->fetch() !== false) {
        return $Qproduct->value_decimal('specials_new_products_price');
    }
    return $result;
}
////
...
function tep_get_products_stock($products_id)
{
    ...
    $Qproduct->bind_int(':products_id', tep_get_prid($products_id));
    $Qproduct->execute();
    return $Qproduct->value_int('products_quantity');
}
////
...
function tep_get_countries($countries_id = '', $with_iso_codes = false)
{
    ...
    if (tep_not_null($countries_id)) {
        if ($with_iso_codes == true) {
            ...
            $Qcountries->bind_int(':countries_id', $countries_id);
            $Qcountries->execute();
            $countries_array = $Qcountries->to_array();
        } else {
        ...
        $countries_array = $OSCOM_Db->query('select countries_id, countries_name from :table_countries order by countries_name')->fetch_all();
    }
...
function tep_get_path(string $current_category_id = ''): string
{
    ...
    if (tep_not_null($current_category_id)) {
        $cp_size = sizeof($c_path_array);
        if ($cp_size == 0) {
            $c_path_new = $current_category_id;
        } else {
            $c_path_new = '';
            $qlast_category = $OSCOM_Db->prepare('select parent_id from :table_categories where categories_id = :categories_id');
            $qlast_category->bind_int(':categories_id', $c_path_array[$cp_size - 1]);
            $qlast_category->execute();
            $qcurrent_category = $OSCOM_Db->prepare('select parent_id from :table_categories where categories_id = :categories_id');
            $qcurrent_category->bind_int(':categories_id', $current_category_id);
            $qcurrent_category->execute();
            if ($qlast_category->value_int('parent_id') == $qcurrent_category->value_int('parent_id')) {
...
function tep_get_zone_name($country_id, $zone_id, $default_zone)
{
    ...
    $Qzone->bind_int(':zone_country_id', $country_id);
    $Qzone->bind_int(':zone_id', $zone_id);
    $Qzone->execute();
...
function tep_get_tax_rate($class_id, $country_id = -1, $zone_id = -1)
{
    ...
    if (!isset($tax_rates[$class_id][$country_id][$zone_id]['rate'])) {
        ...
        $Qtax->bind_int(':zone_country_id', $country_id);
        $Qtax->bind_int(':zone_id', $zone_id);
        $Qtax->bind_int(':tax_class_id', $class_id);
        $Qtax->execute();
        if ($Qtax->fetch() !== false) {
            $tax_multiplier = 1.0;
            do {
                $tax_multiplier *= 1.0 + $Qtax->value_decimal('tax_rate') / 100;
            } while ($Qtax->fetch());
...
function tep_get_tax_description($class_id, $country_id, $zone_id)
{
    ...
    if (!isset($tax_rates[$class_id][$country_id][$zone_id]['description'])) {
        ...
        if ($Qtax->fetch() !== false) {
            ...
            $tax_rates[$class_id][$country_id][$zone_id]['description'] = OSCOM::get_def('text_unknown_tax_rate');
        }
    }
...
function tep_count_products_in_category($category_id, $include_inactive = false): float|int
{
    ...
    $Qproducts->bind_int(':categories_id', $category_id);
    $Qproducts->execute();
    ...
    $Qcategories->bind_int(':parent_id', $category_id);
    $Qcategories->execute();
    if ($Qcategories->fetch() !== false) {
        do {
            $products_count += tep_count_products_in_category($Qcategories->value_int('categories_id'), $include_inactive);
...
function tep_has_category_subcategories($category_id): bool
{
    ...
    $Qcheck->bind_int(':parent_id', $category_id);
    $Qcheck->execute();
...
function tep_get_address_format_id($country_id)
{
    ...
    $Qformat->bind_int(':countries_id', $country_id);
    $Qformat->execute();
    if ($Qformat->fetch() !== false) {
        return $Qformat->value_int('address_format_id');
    }
    return $format_id;
}
////
...
function tep_address_format($address_format_id, array $address, $html, string $boln, $eoln): string
{
    ...
    $Qformat->bind_int(':address_format_id', $address_format_id);
    $Qformat->execute();
    ...
    if (isset($address['country_id']) && tep_not_null($address['country_id'])) {
        ...
        $replace['$country'] = HTML::output_protected($address['country']['title']);
    }
...
function tep_address_label($customers_id, $address_id = 1, $html = false, $boln = '', $eoln = "\n")
{
    ...
    $Qaddress->bind_int(':address_book_id', $address_id);
    $Qaddress->bind_int(':customers_id', $customers_id);
    $Qaddress->execute();
    $format_id = tep_get_address_format_id($Qaddress->value_int('country_id'));
    return tep_address_format($format_id, $Qaddress->to_array(), $html, $boln, $eoln);
}
...
function tep_get_categories($categories_array = '', $parent_id = '0', string $indent = '')
{
    ...
    $Qcategories->bind_int(':parent_id', $parent_id);
    $Qcategories->bind_int(':language_id', $OSCOM_Language->get_id());
    $Qcategories->execute();
    while ($Qcategories->fetch()) {
        $categories_array[] = ['id' => $Qcategories->value_int('categories_id'), 'text' => $indent . $Qcategories->value('categories_name')];
        if ($Qcategories->value_int('categories_id') != $parent_id) {
            $categories_array = tep_get_categories($categories_array, $Qcategories->value_int('categories_id'), $indent . '&nbsp;&nbsp;');
        }
    }
...
function tep_get_subcategories(&$subcategories_array, $parent_id = 0): void
{
    ...
    $Qsub->bind_int(':parent_id', $parent_id);
    $Qsub->execute();
    while ($Qsub->fetch()) {
        $subcategories_array[sizeof($subcategories_array)] = $Qsub->value_int('categories_id');
        if ($Qsub->value_int('categories_id') != $parent_id) {
            tep_get_subcategories($subcategories_array, $Qsub->value_int('categories_id'));
        }
    }
}
////
...
function tep_create_sort_heading($sortby, string $colnum, string $heading): string
{
    ...
    $sort_suffix = '';
    if ($sortby) {
        $sort_prefix = '<a href="' . OSCOM::link($PHP_SELF, tep_get_all_get_params(['page', 'info', 'sort']) . 'page=1&sort=' . $colnum . ($sortby == $colnum . 'a' ? 'd' : 'a')) . '" title="' . HTML::output(OSCOM::get_def('text_sort_products') . ($sortby == $colnum . 'd' || substr((string) $sortby, 0, 1) != $colnum ? OSCOM::get_def('text_ascendingly') : OSCOM::get_def('text_descendingly')) . OSCOM::get_def('text_by') . $heading) . '" class="productListing-heading">';
...
function tep_get_parent_categories(&$categories, $categories_id)
{
    ...
    $Qparent->bind_int(':categories_id', $categories_id);
    $Qparent->execute();
    while ($Qparent->fetch()) {
        if ($Qparent->value_int('parent_id') == 0) {
            return true;
        }
        $categories[sizeof($categories)] = $Qparent->value_int('parent_id');
        if ($Qparent->value_int('parent_id') != $categories_id) {
            tep_get_parent_categories($categories, $Qparent->value_int('parent_id'));
        }
    }
}
////
...
function tep_get_product_path($products_id): string
{
    ...
    $Qcategory->bind_int(':products_id', $products_id);
    $Qcategory->execute();
    if ($Qcategory->fetch() !== false) {
        $categories = [];
        tep_get_parent_categories($categories, $Qcategory->value_int('categories_id'));
        $categories = array_reverse($categories);
        $c_path = implode('_', $categories);
        if (tep_not_null($c_path)) {
            $c_path .= '_';
        }
        $c_path .= $Qcategory->value_int('categories_id');
    }
    return $c_path;
}
////
...
function tep_has_product_attributes($products_id): bool
{
    ...
    $Qattributes->bind_int(':products_id', $products_id);
    $Qattributes->execute();
...
function tep_currency_exists($code)
{
    ...
    $Qcurrency->bind_value(':code', $code);
    $Qcurrency->execute();
...
function tep_count_customer_address_book_entries($id = '', $check_session = true)
{
    ...
    $Qaddresses->bind_int(':customers_id', $id);
    $Qaddresses->execute();
    if ($Qaddresses->fetch() !== false) {
        return $Qaddresses->value_int('total');
    }
    return 0;
}
////
...
{
    $countries_array = [['id' => '', 'text' => OSCOM::get_def('pull_down_default')]];
    $countries = tep_get_countries();
    for ($i = 0, $n = sizeof($countries); $i < $n; $i++) {
        $countries_array[] = ['id' => $countries[$i]['countries_id'], 'text' => $countries[$i]['countries_name']];
    }
    return HTML::select_field($name, $countries_array, $selected, $parameters);
}