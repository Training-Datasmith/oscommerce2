...
function tep_activate_banners(): void
{
    ...
    if ($Qbanners->fetch() !== false) {
        do {
            tep_set_banner_status($Qbanners->value_int('banners_id'), 1);
...
function tep_expire_banners(): void
{
    ...
    if ($Qbanners->fetch() !== false) {
        do {
            tep_set_banner_status($Qbanners->value_int('banners_id'), 0);
        } while ($Qbanners->fetch());
    }
}
////
...
function tep_display_banner($action, $identifier)
{
    ...
    if ($action == 'dynamic') {
        ...
        $Qcheck->bind_value(':banners_group', $identifier);
        $Qcheck->execute();
        if ($Qcheck->fetch() !== false) {
            ...
            $Qbanner->bind_value(':banners_group', $identifier);
            $Qbanner->execute();
        ...
        if (is_array($identifier)) {
            ...
            $Qbanner->bind_int(':banners_id', $identifier);
            $Qbanner->execute();
            if ($Qbanner->fetch() !== false) {
                $banner = $Qbanner->to_array();
            }
        }
    }
    $output = '';
    if (isset($banner)) {
        if (!empty($banner['banners_html_text'])) {
            ...
            $output = '<a href="' . OSCOM::link('redirect.php', 'action=banner&goto=' . $banner['banners_id']) . '" target="_blank">' . HTML::image(OSCOM::link_image($banner['banners_image']), $banner['banners_title']) . '</a>';
        }
...
function tep_banner_exists($action, $identifier)
{
    ...
    if ($action == 'dynamic') {
        ...
        $Qcheck->bind_int(':banners_id', $identifier);
        $Qcheck->execute();
        $result = $Qcheck->fetch() !== false;
    }
    return $result;
...
function tep_update_banner_display_count($banner_id)
{
    ...
    $Qcheck->bind_int(':banners_id', $banner_id);
    $Qcheck->execute();
    if ($Qcheck->fetch() !== false) {
        ...
        $Qview->bind_int(':banners_id', $banner_id);
        $Qview->execute();
    } else {
...
function tep_update_banner_click_count($banner_id)
{
    ...
    $Qupdate->bind_int(':banners_id', $banner_id);
    $Qupdate->execute();
    return $Qupdate->row_count();
}