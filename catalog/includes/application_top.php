...
OSCOM::initialize();
if (!OSCOM::config_exists('db_server') || strlen((string) OSCOM::get_config('db_server')) < 1) {
    if (is_dir('install')) {
...
if (isset($_GET['action'])) {
    ...
    if (Registry::get('Session')->has_started() === false) {
    ...
    switch ($_GET['action']) {
            ...
            for ($i = 0, $n = sizeof($_POST['products_id']); $i < $n; $i++) {
                ...
                if ($_POST['cart_quantity'][$i] > 0) {
                    if ($_POST['cart_quantity'][$i] != $_SESSION['cart']->get_quantity(tep_get_uprid($_POST['products_id'][$i], $attributes))) {
                        ...
                        $message_stack->add_session('product_action', OSCOM::get_def('product_updated', ['products_name' => tep_get_products_name((int) $_POST['products_id'][$i])]), 'success');
                    }
                } else {
                    $_SESSION['cart']->remove($product_id);
                    $message_stack->add_session('product_action', OSCOM::get_def('product_removed', ['products_name' => tep_get_products_name((int) $_POST['products_id'][$i])]), 'warning');
                }
            }
        ...
        case 'add_product':
            if (isset($_POST['products_id']) && is_numeric($_POST['products_id'])) {
                ...
                $message_stack->add_session('product_action', OSCOM::get_def('product_added', ['products_name' => tep_get_products_name((int) $_POST['products_id'])]), 'success');
            }
            OSCOM::redirect($goto, tep_get_all_get_params($parameters));
            break;
        ...
        case 'remove_product':
            if (isset($_GET['products_id'])) {
                ...
                $message_stack->add_session('product_action', OSCOM::get_def('product_removed', ['products_name' => tep_get_products_name($_GET['products_id'])]), 'warning');
            }
            OSCOM::redirect($goto, tep_get_all_get_params($parameters));
            break;
        ...
        case 'buy_now':
            if (isset($_GET['products_id'])) {
                if (tep_has_product_attributes($_GET['products_id'])) {
                    ...
                    $message_stack->add_session('product_action', OSCOM::get_def('product_added', ['products_name' => tep_get_products_name((int) $_GET['products_id'])]), 'success');
                }
            }
            OSCOM::redirect($goto, tep_get_all_get_params($parameters));
            break;
            ...
            if (isset($_SESSION['customer_id'])) {
                ...
                for ($i = 0, $n = sizeof($notify); $i < $n; $i++) {
                    ...
                    if ($Qcheck->fetch() === false) {
                        ...
                        $message_stack->add_session('product_action', OSCOM::get_def('product_subscribed', ['products_name' => tep_get_products_name((int) $notify[$i])]), 'success');
                    }
                }
                OSCOM::redirect($PHP_SELF, tep_get_all_get_params(['action', 'notify']));
            } else {
        ...
        case 'notify_remove':
            if (isset($_SESSION['customer_id']) && isset($_GET['products_id'])) {
                ...
                if ($Qcheck->fetch() !== false) {
                    ...
                    $message_stack->add_session('product_action', OSCOM::get_def('product_unsubscribed', ['products_name' => tep_get_products_name((int) $_GET['products_id'])]), 'warning');
                }
...
if (isset($_GET['cPath'])) {
    ...
    $c_path = tep_get_product_path($_GET['products_id']);
} else {
    $c_path = '';
}
if (!empty($c_path)) {
...
if (isset($c_path_array)) {
    for ($i = 0, $n = sizeof($c_path_array); $i < $n; $i++) {
        $Qcategories = $OSCOM_Db->get('categories_description', 'categories_name', ['categories_id' => $c_path_array[$i], 'language_id' => $OSCOM_Language->get_id()]);
        if ($Qcategories->fetch() !== false) {
            $breadcrumb->add($Qcategories->value('categories_name'), OSCOM::link('index.php', 'cPath=' . implode('_', array_slice($c_path_array, 0, $i + 1))));
        } else {
            break;
        }
    }
