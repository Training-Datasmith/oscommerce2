...
abstract class Session_Abstract
{
    ...
    public function start()
    {
        ...
        if (parse_url((string) OSCOM::get_config('http_server'), PHP_URL_SCHEME) == 'https') {
        ...
        session_set_cookie_params(0, $OSCOM_Cookies->get_path(), $OSCOM_Cookies->get_domain(), (bool) ini_get('session.cookie_secure'), (bool) ini_get('session.cookie_httponly'));
        ...
        if (isset($_COOKIE[$this->name]) && (!(bool) preg_match('/^[a-zA-Z0-9,-]+$/', (string) $_COOKIE[$this->name]) || !$this->exists($_COOKIE[$this->name]))) {
            $OSCOM_Cookies->del($this->name, $OSCOM_Cookies->get_path(), $OSCOM_Cookies->get_domain(), (bool) ini_get('session.cookie_secure'), (bool) ini_get('session.cookie_httponly'));
        }
    ...
    public function set_force_cookies($force_cookies): void
    {
        $this->force_cookies = $force_cookies;
    }
    public function is_force_cookies()
    {
    ...
    public function recreate()
    {
        ...
        if ($result === true) {
            Registry::get('Hooks')->call('Session', 'Recreated', ['old_id' => $session_old_id]);
            return true;
        }
        return false;
    }
