...
class Language
{
    ...
    public function get_id($language_code = null): int
    {
    ...
    public function get_image($language_code, $width = null, $height = null)
    {
    ...
    public function get_client_preference()
    {
    ...
    public function get_def($key, $values = null, $scope = 'global')
    {
        if (isset($this->definitions[$scope][$key])) {
            $def = $this->definitions[$scope][$key];
            if (is_array($values) && !empty($values)) {
                return static::parse_definition($def, $values);
            }
            return $def;
        }
        return $key;
    }
    public static function parse_definition($string, $values)
    {
        if (is_array($values) && !empty($values)) {
    ...
    public function definitions_exist($group, $language_code = null)
    {
        $language_code = isset($language_code) && $this->exists($language_code) ? $language_code : $this->get('code');
        $site = OSCOM::get_site();
        if (str_contains((string) $group, '/') && preg_match('/^([A-Z][A-Za-z0-9-_]*)\/(.*)$/', (string) $group, $matches) === 1 && OSCOM::site_exists($matches[1])) {
            $site = $matches[1];
            $group = $matches[2];
        }
        $pathname = OSCOM::get_config('dir_root', $site) . 'includes/languages/' . $this->get('directory', $language_code) . '/' . $group;
        // legacy
    ...
    public function load_definitions($group, $language_code = null, $scope = null)
    {
        $language_code = isset($language_code) && $this->exists($language_code) ? $language_code : $this->get('code');
        if (!isset($scope)) {
            $scope = 'global';
        ...
        $defs = $this->get_definitions($group, $language_code, $pathname);
        $this->inject_definitions($defs, $scope);
    }
    public function get_definitions($group, $language_code, $pathname)
    {
        $defs = [];
        $group_key = str_replace(['/', '\\'], '-', $group);
        if ($this->use_cache === false) {
            return $this->get_definitions_from_file($pathname);
        }
        $def_cache = new Cache('languages-defs-' . $group_key . '-lang' . $this->get_id($language_code));
        if ($def_cache->exists()) {
            $defs = $def_cache->get();
        } else {
            $Qdefs = $this->db->get('languages_definitions', ['definition_key', 'definition_value'], ['languages_id' => $this->get_id($language_code), 'content_group' => $group_key]);
            while ($Qdefs->fetch()) {
                $defs[$Qdefs->value('definition_key')] = $Qdefs->value('definition_value');
            }
            if (empty($defs)) {
                $defs = $this->get_definitions_from_file($pathname);
                foreach ($defs as $key => $value) {
                    $this->db->save('languages_definitions', ['languages_id' => $this->get_id($language_code), 'content_group' => $group_key, 'definition_key' => $key, 'definition_value' => $value]);
                }
            }
            $def_cache->save($defs);
        }
     ...
     * @return string[]
     */
    public function get_definitions_from_file($filename): array
    {
        $defs = [];
        if (is_file($filename)) {
    ...
    public function inject_definitions($defs, $scope): void
    {
    ...
    public function set_use_cache($flag): void
    {
