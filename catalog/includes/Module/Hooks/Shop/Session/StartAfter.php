...
class Start_After
{
    public function execute(): void
    {
        ...
        if (HTTP::get_request_type() === 'SSL' && SESSION_CHECK_SSL_SESSION_ID == 'True' && $OSCOM_Session->has_started()) {
        ...
        if (SESSION_CHECK_IP_ADDRESS == 'True') {
            if (!isset($_SESSION['SESSION_IP_ADDRESS'])) {
                $_SESSION['SESSION_IP_ADDRESS'] = HTTP::get_ip_address();
            }
            if ($_SESSION['SESSION_IP_ADDRESS'] != HTTP::get_ip_address()) {
                $OSCOM_Session->kill();
                OSCOM::redirect('login.php');
            }
        }
