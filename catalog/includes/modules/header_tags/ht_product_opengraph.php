...
class ht_product_opengraph
{
    ...
    {
        $this->title = OSCOM::get_def('module_header_tags_product_opengraph_title');
        $this->description = OSCOM::get_def('module_header_tags_product_opengraph_description');
    ...
    public function execute(): void
    {
        ...
        if (basename((string) $PHP_SELF) == 'product_info.php') {
            ...
            if ($Qproduct->fetch() !== false) {
                $data = ['og:type' => 'product', 'og:title' => $Qproduct->value('products_name'), 'og:site_name' => STORE_NAME];
                ...
                $data['og:image'] = OSCOM::link_image($products_image);
                ...
                $data['og:url'] = OSCOM::link('product_info.php', 'products_id=' . $Qproduct->value_int('products_id'), false);
                $data['product:availability'] = $Qproduct->value_int('products_quantity') > 0 ? OSCOM::get_def('module_header_tags_product_opengraph_text_in_stock') : OSCOM::get_def('module_header_tags_product_opengraph_text_out_of_stock');
                $result = '';
                foreach ($data as $key => $value) {
                    $result .= '<meta property="' . HTML::output_protected($key) . '" content="' . HTML::output_protected($value) . '" />' . PHP_EOL;
                }
    ...
    {
        $OSCOM_Db = Registry::get('Db');
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Enable Product OpenGraph Module', 'configuration_key' => 'MODULE_HEADER_TAGS_PRODUCT_OPENGRAPH_STATUS', 'configuration_value' => 'True', 'configuration_description' => 'Do you want to allow Open Graph Meta Tags (good for Facebook and Pinterest and other sites) to be added to your product page?  Note that your product thumbnails MUST be at least 200px by 200px.', 'configuration_group_id' => '6', 'sort_order' => '1', 'set_function' => 'tep_cfg_select_option(array(\'True\', \'False\'), ', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Sort Order', 'configuration_key' => 'MODULE_HEADER_TAGS_PRODUCT_OPENGRAPH_SORT_ORDER', 'configuration_value' => '900', 'configuration_description' => 'Sort order of display. Lowest is displayed first.', 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()']);
    }
