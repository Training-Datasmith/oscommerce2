...
class ht_grid_list_view
{
    ...
    {
        $this->title = OSCOM::get_def('module_header_tags_grid_list_view_title');
        $this->description = OSCOM::get_def('module_header_tags_grid_list_view_description');
    ...
    public function install(): void
    {
        ...
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Enable Grid List javascript', 'configuration_key' => 'MODULE_HEADER_TAGS_GRID_LIST_VIEW_STATUS', 'configuration_value' => 'True', 'configuration_description' => 'Do you want to enable the Grid/List Javascript module?', 'configuration_group_id' => '6', 'sort_order' => '1', 'set_function' => 'tep_cfg_select_option(array(\'True\', \'False\'), ', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Pages', 'configuration_key' => 'MODULE_HEADER_TAGS_GRID_LIST_VIEW_PAGES', 'configuration_value' => implode(';', $this->get_default_pages()), 'configuration_description' => 'The pages to add the Grid List JS Scripts to.', 'configuration_group_id' => '6', 'sort_order' => '1', 'use_function' => 'ht_grid_list_view_show_pages', 'set_function' => 'ht_grid_list_view_edit_pages(', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Sort Order', 'configuration_key' => 'MODULE_HEADER_TAGS_GRID_LIST_VIEW_SORT_ORDER', 'configuration_value' => '0', 'configuration_description' => 'Sort order of display. Lowest is displayed first.', 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()']);
    }
    ...
    {
        return ['advanced_search_result.php', 'index.php', 'products_new.php', 'specials.php'];
    }
}
...
function ht_grid_list_view_edit_pages($values, string $key): string
{
    ...
    foreach ($files_array as $file) {
        $output .= HTML::checkbox_field('ht_grid_list_view_file[]', $file, in_array($file, $values_array)) . '&nbsp;' . HTML::output($file) . '<br />';
    }
    if (!empty($output)) {
