...
?> category-new-products">
  ...
  <h3><?php 
echo OSCOM::get_def('module_content_in_new_products_heading', ['current_month' => strftime('%B')]);
?></h3>
...
foreach ($new_products as $product) {
    ...
    ?>"><?php 
    echo HTML::image(OSCOM::link_image($product['products_image']), $product['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'itemprop="image"');
    ?></a>
        <div class="caption">
    ...
    ?></span></p>
          <div class="text-center">
            <div class="btn-group">
              <a href="<?php 
    echo OSCOM::link('product_info.php', tep_get_all_get_params(['action']) . 'products_id=' . (int) $product['products_id']);
    ?>" class="btn btn-default" role="button"><?php 
    echo OSCOM::get_def('module_content_in_new_products_button_view');
    ?></a>
              <a href="<?php 
    echo OSCOM::link($PHP_SELF, tep_get_all_get_params(['action']) . 'action=buy_now&products_id=' . (int) $product['products_id']);
    ?>" class="btn btn-success btn-index-nested btn-buy" role="button"><?php 
    echo OSCOM::get_def('module_content_in_new_products_button_buy');
    ?></a>
            </div>
          </div>
        </div>
