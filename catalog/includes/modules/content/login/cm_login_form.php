...
{
    /**
     * @var class-string<\cm_login_form>
     */
    public $code;
    /**
     * @var string
    ...
    public function __construct()
    {
        ...
        $this->title = OSCOM::get_def('module_content_login_form_title');
        $this->description = OSCOM::get_def('module_content_login_form_description');
    ...
    {
        global $login_customer_id, $message_stack, $osc_template;
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'process' && isset($_POST['formid']) && $_POST['formid'] == $_SESSION['sessiontoken']) {
            ...
            if ($Qcustomer->fetch() === false) {
                $error = true;
            } else if (!Hash::verify($password, $Qcustomer->value('customers_password'))) {
                $error = true;
            } else {
                // set $login_customer_id globally and perform post login code in catalog/login.php
                $login_customer_id = $Qcustomer->value_int('customers_id');
                // migrate old hashed password to new php password_hash
                if (Hash::needs_rehash($Qcustomer->value('customers_password'))) {
                    $OSCOM_Db->save('customers', ['customers_password' => Hash::encrypt($password)], ['customers_id' => $login_customer_id]);
                }
            }
        }
        if ($error == true) {
            $message_stack->add('login', OSCOM::get_def('module_content_login_text_login_error'));
        }
        ob_start();
    ...
    {
        return ['MODULE_CONTENT_LOGIN_FORM_STATUS', 'MODULE_CONTENT_LOGIN_FORM_CONTENT_WIDTH', 'MODULE_CONTENT_LOGIN_FORM_SORT_ORDER'];
    }
}