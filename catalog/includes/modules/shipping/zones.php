...
class zones
{
    ...
    {
        $this->code = 'zones';
        $this->title = OSCOM::get_def('module_shipping_zones_text_title');
        $this->description = OSCOM::get_def('module_shipping_zones_text_description');
    ...
    public function quote($method = '')
    {
        ...
        if ($dest_zone == 0) {
            ...
            for ($i = 0; $i < $size; $i += 2) {
                if ($shipping_weight <= $zones_table[$i]) {
                    ...
                    $shipping_method = OSCOM::get_def('module_shipping_zones_text_way') . ' ' . $dest_country . ' : ' . $shipping_weight . ' ' . OSCOM::get_def('module_shipping_zones_text_units');
                    break;
                }
            }
            if ($shipping == -1) {
                $shipping_cost = 0;
                $shipping_method = OSCOM::get_def('module_shipping_zones_undefined_rate');
            } else {
                $shipping_cost = $shipping * $shipping_num_boxes + constant('MODULE_SHIPPING_ZONES_HANDLING_' . $dest_zone);
            }
        }
        $this->quotes = ['id' => $this->code, 'module' => OSCOM::get_def('module_shipping_zones_text_title'), 'methods' => [['id' => $this->code, 'title' => $shipping_method, 'cost' => $shipping_cost]]];
        if ($this->tax_class > 0) {
        ...
        if ($error == true) {
            $this->quotes['error'] = OSCOM::get_def('module_shipping_zones_invalid_zone');
        }
        return $this->quotes;
    }
    ...
    public function install(): void
    {
        ...
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Enable Zones Method', 'configuration_key' => 'MODULE_SHIPPING_ZONES_STATUS', 'configuration_value' => 'True', 'configuration_description' => 'Do you want to offer zone rate shipping?', 'configuration_group_id' => '6', 'sort_order' => '1', 'set_function' => 'tep_cfg_select_option(array(\'True\', \'False\'), ', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Tax Class', 'configuration_key' => 'MODULE_SHIPPING_ZONES_TAX_CLASS', 'configuration_value' => '0', 'configuration_description' => 'Use the following tax class on the shipping fee.', 'configuration_group_id' => '6', 'sort_order' => '1', 'use_function' => 'tep_get_tax_class_title', 'set_function' => 'tep_cfg_pull_down_tax_classes(', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Sort Order', 'configuration_key' => 'MODULE_SHIPPING_ZONES_SORT_ORDER', 'configuration_value' => '0', 'configuration_description' => 'Sort order of display. Lowest is displayed first.', 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()']);
        for ($i = 1; $i <= $this->num_zones; $i++) {
            $default_countries = '';
            if ($i == 1) {
                $default_countries = 'US,CA';
            }
            $OSCOM_Db->save('configuration', ['configuration_title' => 'Zone ' . $i . ' Countries', 'configuration_key' => 'MODULE_SHIPPING_ZONES_COUNTRIES_' . $i, 'configuration_value' => $default_countries, 'configuration_description' => 'Comma separated list of two character ISO country codes that are part of Zone ' . $i . '.', 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()']);
            $OSCOM_Db->save('configuration', ['configuration_title' => 'Zone ' . $i . ' Shipping Table', 'configuration_key' => 'MODULE_SHIPPING_ZONES_COST_' . $i, 'configuration_value' => '3:8.50,7:10.50,99:20.00', 'configuration_description' => 'Shipping rates to Zone ' . $i . ' destinations based on a group of maximum order weights. Example: 3:8.50,7:10.50,... Weights less than or equal to 3 would cost 8.50 for Zone ' . $i . ' destinations.', 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()']);
            $OSCOM_Db->save('configuration', ['configuration_title' => 'Zone ' . $i . ' Handling Fee', 'configuration_key' => 'MODULE_SHIPPING_ZONES_HANDLING_' . $i, 'configuration_value' => '0', 'configuration_description' => 'Handling Fee for this shipping zone', 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()']);
        }
    }
