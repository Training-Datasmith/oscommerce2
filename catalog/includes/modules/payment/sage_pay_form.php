...
class sage_pay_form
{
    ...
    public function __construct()
    {
        ...
        $this->title = OSCOM::get_def('module_payment_sage_pay_form_text_title');
        $this->public_title = OSCOM::get_def('module_payment_sage_pay_form_text_public_title');
        $this->description = OSCOM::get_def('module_payment_sage_pay_form_text_description');
        ...
        if (!function_exists('mcrypt_encrypt')) {
            $this->description = '<div class="secWarning">' . OSCOM::get_def('module_payment_sage_pay_form_error_admin_mcrypt') . '</div>' . $this->description;
            $this->enabled = false;
        }
        if ($this->enabled === true) {
            if (!tep_not_null(MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_LOGIN_NAME) || !tep_not_null(MODULE_PAYMENT_SAGE_PAY_FORM_ENCRYPTION_PASSWORD)) {
                $this->description = '<div class="secWarning">' . OSCOM::get_def('module_payment_sage_pay_form_error_admin_configuration') . '</div>' . $this->description;
                $this->enabled = false;
            }
        }
        if ($this->enabled === true) {
    ...
    public function process_button(): string
    {
        ...
        $params = ['VPSProtocol' => $this->api_version, 'Vendor' => substr(MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_LOGIN_NAME, 0, 15)];
        ...
        foreach ($this->get_order_totals_summary() as $ot) {
        ...
        $params['Crypt'] = $this->encrypt_params($crypt_string);
        foreach ($params as $key => $value) {
            $process_button_string .= HTML::hidden_field($key, $value);
        }
    ...
    public function before_process(): void
    {
        ...
        if (isset($_GET['crypt']) && tep_not_null($_GET['crypt'])) {
            $transaction_response = $this->decrypt_params($_GET['crypt']);
    ...
    public function after_process(): void
    {
        ...
        $sql_data_array = ['orders_id' => $insert_id, 'orders_status_id' => MODULE_PAYMENT_SAGE_PAY_FORM_TRANSACTION_ORDER_STATUS_ID, 'date_added' => 'now()', 'customer_notified' => '0', 'comments' => trim($result_string)];
        $OSCOM_Db->save('orders_status_history', $sql_data_array);
    }
    public function get_error(): false|array
    {
        $message = OSCOM::get_def('module_payment_sage_pay_form_error_general');
        $error_number = null;
        if (isset($_GET['error']) && is_numeric($_GET['error']) && $this->error_message_number_exists($_GET['error'])) {
            ...
            if (is_numeric($error) && $this->error_message_number_exists($error)) {
                $error_number = $error;
            }
        }
        if (isset($error_number)) {
            ...
            $message = $this->get_error_message($error_number) . ' ' . OSCOM::get_def('module_payment_sage_pay_form_error_general');
        }
        return ['title' => OSCOM::get_def('module_payment_sage_pay_form_error_title'), 'error' => $message];
    }
    ...
    public function get_params(): array
    {
        ...
        return ['MODULE_PAYMENT_SAGE_PAY_FORM_STATUS' => ['title' => 'Enable Sage Pay Form Module', 'desc' => 'Do you want to accept Sage Pay Form payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_LOGIN_NAME' => ['title' => 'Vendor Login Name', 'desc' => 'The vendor login name to connect to the gateway with.'], 'MODULE_PAYMENT_SAGE_PAY_FORM_ENCRYPTION_PASSWORD' => ['title' => 'Encryption Password', 'desc' => 'The encrpytion password to secure and verify transactions with.'], 'MODULE_PAYMENT_SAGE_PAY_FORM_TRANSACTION_METHOD' => ['title' => 'Transaction Method', 'desc' => 'The processing method to use for each transaction.', 'value' => 'Authenticate', 'set_func' => 'tep_cfg_select_option(array(\'Authenticate\', \'Deferred\', \'Payment\'), '], 'MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_EMAIL' => ['title' => 'Vendor E-Mail Notification', 'desc' => 'An e-mail address on which you can be contacted when a transaction completes. NOTE: If you wish to use multiple email addresses, you should add them using the colon character as a separator. e.g. me@mail1.com:me@mail2.com'], 'MODULE_PAYMENT_SAGE_PAY_FORM_SEND_EMAIL' => ['title' => 'Send E-Mail Notifications', 'desc' => 'Who to send e-mails to.', 'value' => 'Customer and Vendor', 'set_func' => 'tep_cfg_select_option(array(\'No One\', \'Customer and Vendor\', \'Vendor Only\'), '], 'MODULE_PAYMENT_SAGE_PAY_FORM_CUSTOMER_EMAIL_MESSAGE' => ['title' => 'Customer E-Mail Message', 'desc' => 'A message to the customer which is inserted into successful transaction e-mails only.', 'use_func' => 'sage_pay_form_clip_text', 'set_func' => 'sage_pay_form_textarea_field('], 'MODULE_PAYMENT_SAGE_PAY_FORM_ORDER_STATUS_ID' => ['title' => 'Set Order Status', 'desc' => 'Set the status of orders made with this payment module to this value', 'value' => '0', 'use_func' => 'tep_get_order_status_name', 'set_func' => 'tep_cfg_pull_down_order_statuses('], 'MODULE_PAYMENT_SAGE_PAY_FORM_TRANSACTION_ORDER_STATUS_ID' => ['title' => 'Transaction Order Status', 'desc' => 'Include transaction information in this order status level', 'value' => $status_id, 'set_func' => 'tep_cfg_pull_down_order_statuses(', 'use_func' => 'tep_get_order_status_name'], 'MODULE_PAYMENT_SAGE_PAY_FORM_ZONE' => ['title' => 'Payment Zone', 'desc' => 'If a zone is selected, only enable this payment method for that zone.', 'value' => '0', 'use_func' => 'tep_get_zone_class_title', 'set_func' => 'tep_cfg_pull_down_zone_classes('], 'MODULE_PAYMENT_SAGE_PAY_FORM_TRANSACTION_SERVER' => ['title' => 'Transaction Server', 'desc' => 'Perform transactions on the production server or on the testing server.', 'value' => 'Live', 'set_func' => 'tep_cfg_select_option(array(\'Live\', \'Test\'), '], 'MODULE_PAYMENT_SAGE_PAY_FORM_DEBUG_EMAIL' => ['title' => 'Debug E-Mail Address', 'desc' => 'All parameters of an invalid transaction will be sent to this email address.'], 'MODULE_PAYMENT_SAGE_PAY_FORM_SORT_ORDER' => ['title' => 'Sort order of display.', 'desc' => 'Sort order of display. Lowest is displayed first.', 'value' => '0']];
    }
     ...
     */
    public function get_order_totals_summary(): array
    {
                ...
                if ($GLOBALS[$class]->enabled) {
                    for ($i = 0, $n = sizeof($GLOBALS[$class]->output); $i < $n; $i++) {
                        if (tep_not_null($GLOBALS[$class]->output[$i]['title']) && tep_not_null($GLOBALS[$class]->output[$i]['text'])) {
                            $order_total_array[] = ['code' => $GLOBALS[$class]->code, 'title' => $GLOBALS[$class]->output[$i]['title'], 'text' => $GLOBALS[$class]->output[$i]['text'], 'value' => $GLOBALS[$class]->output[$i]['value'], 'sort_order' => $GLOBALS[$class]->sort_order];
                        }
                    }
                }
            }
    ...
    public function encrypt_params(string $string): string
    {
        // pad pkcs5
        $blocksize = 16;
    ...
    public function decrypt_params($string)
    {
    ...
    public function send_debug_email($response = []): void
    {
        if (tep_not_null(MODULE_PAYMENT_SAGE_PAY_FORM_DEBUG_EMAIL)) {
            ...
            if (!empty($email_body)) {
                $debug_email = new Mail(MODULE_PAYMENT_SAGE_PAY_FORM_DEBUG_EMAIL, null, STORE_OWNER_EMAIL_ADDRESS, STORE_OWNER, 'Sage Pay Form Debug E-Mail');
...
{
    return HTML::textarea_field('configuration[' . $key . ']', 60, 5, $value);
}