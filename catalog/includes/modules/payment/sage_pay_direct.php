...
class sage_pay_direct
{
    ...
    public function __construct()
    {
        ...
        $this->title = OSCOM::get_def('module_payment_sage_pay_direct_text_title');
        $this->public_title = OSCOM::get_def('module_payment_sage_pay_direct_text_public_title');
        $this->description = OSCOM::get_def('module_payment_sage_pay_direct_text_description');
        ...
        if (defined('MODULE_PAYMENT_SAGE_PAY_DIRECT_STATUS')) {
            ...
            $this->description .= $this->get_test_link_info();
        }
        if (!function_exists('curl_init')) {
            $this->description = '<div class="secWarning">' . OSCOM::get_def('module_payment_sage_pay_direct_error_admin_curl') . '</div>' . $this->description;
            $this->enabled = false;
        }
        if ($this->enabled === true) {
            if (!tep_not_null(MODULE_PAYMENT_SAGE_PAY_DIRECT_VENDOR_LOGIN_NAME)) {
                $this->description = '<div class="secWarning">' . OSCOM::get_def('module_payment_sage_pay_direct_error_admin_configuration') . '</div>' . $this->description;
                $this->enabled = false;
            }
        }
        if ($this->enabled === true) {
        ...
        if (defined('FILENAME_MODULES') && basename((string) $PHP_SELF) == 'modules.php' && isset($_GET['action']) && $_GET['action'] == 'install' && isset($_GET['subaction']) && $_GET['subaction'] == 'conntest') {
            echo $this->get_test_connection_result();
            exit;
        }
    }
    public function update_status(): void
    {
        ...
        if ($this->enabled == true && $this->has_cards() == false) {
            $this->enabled = false;
        }
        if ($this->enabled == true && (int) MODULE_PAYMENT_SAGE_PAY_DIRECT_ZONE > 0) {
            ...
            while ($Qcheck->fetch()) {
                if ($Qcheck->value_int('zone_id') < 1) {
                    $check_flag = true;
                    break;
                } elseif ($Qcheck->value_int('zone_id') == $order->billing['zone_id']) {
                    $check_flag = true;
                    break;
                }
            }
    ...
    public function selection(): array
    {
        ...
        return ['id' => $this->code, 'module' => $this->public_title];
    }
    public function pre_confirmation_check(): void
    {
        if ($this->template_class_exists()) {
            $GLOBALS['oscTemplate']->add_block($this->get_submit_card_details_javascript(), 'header_tags');
        }
    }
    public function confirmation(): array
    {
        ...
        $OSCOM_Db = Registry::get('Db');
        $card_types = [];
        foreach ($this->get_card_types() as $key => $value) {
            $card_types[] = ['id' => $key, 'text' => $value];
        }
        $today = getdate();
        $months_array = [];
        for ($i = 1; $i < 13; $i++) {
        ...
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_TOKENS == 'True') {
            ...
            if ($Qtokens->fetch() !== false) {
                ...
                do {
                    $content .= '<tr class="moduleRow" id="sagepay_card_' . $Qtokens->value_int('id') . '">' . '  <td width="40" valign="top"><input type="radio" name="sagepay_card" value="' . $Qtokens->value_int('id') . '" /></td>' . '  <td valign="top">' . $Qtokens->value_protected('number_filtered') . '&nbsp;&nbsp;' . HTML::output_protected(substr((string) $Qtokens->value('expiry_date'), 0, 2)) . '/' . strftime('%Y', mktime(0, 0, 0, 1, 1, 2000 + substr((string) $Qtokens->value('expiry_date'), 2))) . '&nbsp;&nbsp;' . $Qtokens->value_protected('card_type') . '</td>' . '</tr>';
                    if (MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_WITH_CVC == 'True') {
                        $content .= '<tr class="moduleRowExtra" id="sagepay_card_cvc_' . $Qtokens->value_int('id') . '">' . '  <td width="40" valign="top">&nbsp;</td>' . '  <td valign="top">' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_cvc') . '&nbsp;' . HTML::input_field('cc_cvc_tokens_nh-dns[' . $Qtokens->value_int('id') . ']', '', 'size="5" maxlength="4"') . '</td>' . '</tr>';
                    }
                } while ($Qtokens->fetch());
                $content .= '<tr class="moduleRow" id="sagepay_card_0">' . '  <td width="40" valign="top"><input type="radio" name="sagepay_card" value="0" /></td>' . '  <td valign="top">' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_new') . '</td>' . '</tr>' . '</table>';
            }
        }
        $content .= '<table id="sagepay_table_new_card" border="0" width="100%" cellspacing="0" cellpadding="2">' . '<tr>' . '  <td width="30%">' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_type') . '</td>' . '  <td>' . HTML::select_field('cc_type', $card_types, '', 'id="sagepay_card_type"') . '</td>' . '</tr>' . '<tr>' . '  <td width="30%">' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_owner') . '</td>' . '  <td>' . HTML::input_field('cc_owner', $order->billing['firstname'] . ' ' . $order->billing['lastname'], 'maxlength="50"') . '</td>' . '</tr>' . '<tr>' . '  <td width="30%">' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_number') . '</td>' . '  <td>' . HTML::input_field('cc_number_nh-dns', '', 'maxlength="20"') . '</td>' . '</tr>';
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO == 'True' || MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_AMEX == 'True') {
            $content .= '<tr>' . '  <td width="30%">' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_starts') . '</td>' . '  <td>' . HTML::select_field('cc_starts_month', $months_array, '', 'id="sagepay_card_date_start"') . '&nbsp;' . HTML::select_field('cc_starts_year', $year_valid_from_array) . '&nbsp;' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_starts_info') . '</td>' . '</tr>';
        }
        $content .= '<tr>' . '  <td width="30%">' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_expires') . '</td>' . '  <td>' . HTML::select_field('cc_expires_month', $months_array) . '&nbsp;' . HTML::select_field('cc_expires_year', $year_valid_to_array) . '</td>' . '</tr>';
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO == 'True') {
            $content .= '<tr>' . '  <td width="30%">' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_issue_number') . '</td>' . '  <td>' . HTML::input_field('cc_issue_nh-dns', '', 'id="sagepay_card_issue" size="3" maxlength="2"') . '&nbsp;' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_issue_number_info') . '</td>' . '</tr>';
        }
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_WITH_CVC == 'True') {
            $content .= '<tr>' . '  <td width="30%">' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_cvc') . '</td>' . '  <td>' . HTML::input_field('cc_cvc_nh-dns', '', 'size="5" maxlength="4"') . '</td>' . '</tr>';
        }
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_TOKENS == 'True') {
            $content .= '<tr>' . '  <td width="30%">&nbsp;</td>' . '  <td>' . HTML::checkbox_field('cc_save', 'true') . ' ' . OSCOM::get_def('module_payment_sage_pay_direct_credit_card_save') . '</td>' . '</tr>';
        }
        $content .= '</table>';
        $content .= !$this->template_class_exists() ? $this->get_submit_card_details_javascript() : '';
    ...
    public function before_process(): void
    {
        ...
        if (isset($_GET['check'])) {
            if ($_GET['check'] == '3D' && isset($_POST['MD']) && tep_not_null($_POST['MD']) && isset($_POST['PaRes']) && tep_not_null($_POST['PaRes'])) {
                ...
                $transaction_response = $this->send_transaction_to_gateway($gateway_url, $post_string);
                ...
                if ($_POST['Status'] == 'PAYPALOK' && isset($_POST['VPSTxId']) && isset($_POST['CustomerEMail']) && isset($_POST['PayerID'])) {
                    $params = ['VPSProtocol' => $this->api_version, 'TxType' => 'COMPLETE', 'VPSTxId' => $_POST['VPSTxId'], 'Amount' => $this->format_raw($order->info['total']), 'Accept' => 'YES'];
                    $post_string = '';
                ...
                if (isset($_POST['sagepay_card']) && is_numeric($_POST['sagepay_card']) && $_POST['sagepay_card'] > 0) {
                    ...
                    if ($Qtoken->fetch() !== false) {
                        ...
                        if (isset($_POST['cc_cvc_tokens_nh-dns']) && is_array($_POST['cc_cvc_tokens_nh-dns']) && isset($_POST['cc_cvc_tokens_nh-dns'][$_POST['sagepay_card']])) {
                            $sagepay_token_cvc = substr((string) $_POST['cc_cvc_tokens_nh-dns'][$_POST['sagepay_card']], 0, 4);
                        }
                    }
                }
            }
            if (!isset($sagepay_token)) {
                $cc_type = isset($_POST['cc_type']) ? substr((string) $_POST['cc_type'], 0, 15) : null;
                if (!isset($cc_type) || $this->is_card($cc_type) == false) {
                    OSCOM::redirect('checkout_payment.php', 'payment_error=' . $this->code . '&error=cardtype');
                }
                if ($cc_type != 'PAYPAL') {
                    $cc_owner = isset($_POST['cc_owner']) ? substr((string) $_POST['cc_owner'], 0, 50) : null;
                    $cc_number = isset($_POST['cc_number_nh-dns']) ? substr((string) preg_replace('/[^0-9]/', '', (string) $_POST['cc_number_nh-dns']), 0, 20) : null;
                    $cc_start = null;
                    $cc_expires = null;
                    $cc_issue = isset($_POST['cc_issue_nh-dns']) ? substr((string) $_POST['cc_issue_nh-dns'], 0, 2) : null;
                    $cc_cvc = isset($_POST['cc_cvc_nh-dns']) ? substr((string) $_POST['cc_cvc_nh-dns'], 0, 4) : null;
                    $today = getdate();
                    $months_array = [];
                    for ($i = 1; $i < 13; $i++) {
            ...
            $params = ['VPSProtocol' => $this->api_version, 'ReferrerID' => 'C74D7B82-E9EB-4FBD-93DB-76F0F551C802', 'Vendor' => substr(MODULE_PAYMENT_SAGE_PAY_DIRECT_VENDOR_LOGIN_NAME, 0, 15), 'VendorTxCode' => substr(date('YmdHis') . '-' . $_SESSION['customer_id'] . '-' . $_SESSION['cartID'], 0, 40), 'Amount' => $this->format_raw($order->info['total']), 'Currency' => $_SESSION['currency'], 'Description' => substr(STORE_NAME, 0, 100), 'BillingSurname' => substr((string) $order->billing['lastname'], 0, 20), 'BillingFirstnames' => substr((string) $order->billing['firstname'], 0, 20), 'BillingAddress1' => substr((string) $order->billing['street_address'], 0, 100), 'BillingCity' => substr((string) $order->billing['city'], 0, 40), 'BillingPostCode' => substr((string) $order->billing['postcode'], 0, 10), 'BillingCountry' => $order->billing['country']['iso_code_2'], 'BillingPhone' => substr((string) $order->customer['telephone'], 0, 20), 'DeliverySurname' => substr((string) $order->delivery['lastname'], 0, 20), 'DeliveryFirstnames' => substr((string) $order->delivery['firstname'], 0, 20), 'DeliveryAddress1' => substr((string) $order->delivery['street_address'], 0, 100), 'DeliveryCity' => substr((string) $order->delivery['city'], 0, 40), 'DeliveryPostCode' => substr((string) $order->delivery['postcode'], 0, 10), 'DeliveryCountry' => $order->delivery['country']['iso_code_2'], 'DeliveryPhone' => substr((string) $order->customer['telephone'], 0, 20), 'CustomerEMail' => substr((string) $order->customer['email_address'], 0, 255), 'Apply3DSecure' => '0', 'VendorData' => 'Customer ID ' . $_SESSION['customer_id']];
            if (isset($sagepay_token)) {
            ...
            $ip_address = HTTP::get_ip_address();
        ...
        $string_array = explode(chr(10), (string) $transaction_response);
        $sage_pay_response = [];
        ...
        if (isset($params['CreateToken']) && $params['CreateToken'] == '1') {
            ...
            $_SESSION['sagepay_token_cc_number'] = str_repeat('X', strlen((string) $params['CardNumber']) - 4) . substr((string) $params['CardNumber'], -4);
        ...
        if ($sage_pay_response['Status'] != 'OK' && $sage_pay_response['Status'] != 'AUTHENTICATED' && $sage_pay_response['Status'] != 'REGISTERED') {
            $this->send_debug_email($sage_pay_response);
            $error = $this->get_error_message_number($sage_pay_response['StatusDetail']);
    ...
    public function after_process(): void
    {
        ...
        if (isset($sage_pay_response['Token']) && isset($_SESSION['sagepay_token_cc_number'])) {
            ...
            if ($Qcheck->fetch() === false) {
                $sql_data_array = ['customers_id' => $_SESSION['customer_id'], 'sagepay_token' => $sage_pay_response['Token'], 'card_type' => $_SESSION['sagepay_token_cc_type'], 'number_filtered' => $_SESSION['sagepay_token_cc_number'], 'expiry_date' => $_SESSION['sagepay_token_cc_expiry_date'], 'date_added' => 'now()'];
        ...
        $sql_data_array = ['orders_id' => $insert_id, 'orders_status_id' => MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_ORDER_STATUS_ID, 'date_added' => 'now()', 'customer_notified' => '0', 'comments' => trim($result_string)];
    ...
    {
        $message = OSCOM::get_def('module_payment_sage_pay_direct_error_general');
        if (isset($_GET['error']) && tep_not_null($_GET['error'])) {
            if (is_numeric($_GET['error']) && $this->error_message_number_exists($_GET['error'])) {
                $message = $this->get_error_message($_GET['error']) . ' ' . OSCOM::get_def('module_payment_sage_pay_direct_error_general');
            } else {
                switch ($_GET['error']) {
                    case 'cardtype':
                        $message = OSCOM::get_def('module_payment_sage_pay_direct_error_cardtype');
                        break;
                    case 'cardowner':
                        $message = OSCOM::get_def('module_payment_sage_pay_direct_error_cardowner');
                        break;
                    case 'cardnumber':
                        $message = OSCOM::get_def('module_payment_sage_pay_direct_error_cardnumber');
                        break;
                    case 'cardstart':
                        $message = OSCOM::get_def('module_payment_sage_pay_direct_error_cardstart');
                        break;
                    case 'cardexpires':
                        $message = OSCOM::get_def('module_payment_sage_pay_direct_error_cardexpires');
                        break;
                    case 'cardissue':
                        $message = OSCOM::get_def('module_payment_sage_pay_direct_error_cardissue');
                        break;
                    case 'cardcvc':
                        $message = OSCOM::get_def('module_payment_sage_pay_direct_error_cardcvc');
                        break;
                }
            }
        }
        return ['title' => OSCOM::get_def('module_payment_sage_pay_direct_error_title'), 'error' => $message];
    }
    ...
    public function install($parameter = null): void
    {
        ...
        foreach ($params as $key => $data) {
            $sql_data_array = ['configuration_title' => $data['title'], 'configuration_key' => $key, 'configuration_value' => $data['value'] ?? '', 'configuration_description' => $data['desc'], 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()'];
    ...
    public function get_params(): array
    {
        ...
        if (!defined('MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_ORDER_STATUS_ID')) {
            ...
            if ($Qcheck->fetch() === false) {
                ...
                $status_id = $Qstatus->value_int('status_id') + 1;
                ...
                $status_id = $Qcheck->value_int('orders_status_id');
            }
        } else {
            $status_id = MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_ORDER_STATUS_ID;
        }
        return ['MODULE_PAYMENT_SAGE_PAY_DIRECT_STATUS' => ['title' => 'Enable Sage Pay Direct Module', 'desc' => 'Do you want to accept Sage Pay Direct payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_VENDOR_LOGIN_NAME' => ['title' => 'Vendor Login Name', 'desc' => 'The vendor login name to connect to the gateway with.', 'value' => ''], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_WITH_CVC' => ['title' => 'Verify With CVC', 'desc' => 'Verify the credit card with the billing address with the Credit Card Verification Checknumber (CVC)?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_TOKENS' => ['title' => 'Create Tokens', 'desc' => 'Create and store tokens for card payments customer can use on their next purchase?', 'value' => 'False', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_METHOD' => ['title' => 'Transaction Method', 'desc' => 'The processing method to use for each transaction.', 'value' => 'Authenticate', 'set_func' => 'tep_cfg_select_option(array(\'Authenticate\', \'Deferred\', \'Payment\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ORDER_STATUS_ID' => ['title' => 'Set Order Status', 'desc' => 'Set the status of orders made with this payment module to this value', 'value' => '0', 'use_func' => 'tep_get_order_status_name', 'set_func' => 'tep_cfg_pull_down_order_statuses('], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_ORDER_STATUS_ID' => ['title' => 'Transaction Order Status', 'desc' => 'Include transaction information in this order status level', 'value' => $status_id, 'set_func' => 'tep_cfg_pull_down_order_statuses(', 'use_func' => 'tep_get_order_status_name'], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ZONE' => ['title' => 'Payment Zone', 'desc' => 'If a zone is selected, only enable this payment method for that zone.', 'value' => '0', 'use_func' => 'tep_get_zone_class_title', 'set_func' => 'tep_cfg_pull_down_zone_classes('], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_SERVER' => ['title' => 'Transaction Server', 'desc' => 'Perform transactions on the production server or on the testing server.', 'value' => 'Live', 'set_func' => 'tep_cfg_select_option(array(\'Live\', \'Test\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_SSL' => ['title' => 'Verify SSL Certificate', 'desc' => 'Verify transaction server SSL certificate on connection?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_PROXY' => ['title' => 'Proxy Server', 'desc' => 'Send API requests through this proxy server. (host:port, eg: 123.45.67.89:8080 or proxy.example.com:8080)'], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_DEBUG_EMAIL' => ['title' => 'Debug E-Mail Address', 'desc' => 'All parameters of an invalid transaction will be sent to this email address.'], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_SORT_ORDER' => ['title' => 'Sort order of display.', 'desc' => 'Sort order of display. Lowest is displayed first.', 'value' => '0'], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_VISA' => ['title' => 'Accept Visa', 'desc' => 'Do you want to accept Visa payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MC' => ['title' => 'Accept Mastercard', 'desc' => 'Do you want to accept Mastercard payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MCDEBIT' => ['title' => 'Accept Mastercard Debit', 'desc' => 'Do you want to accept Mastercard Debit payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_DELTA' => ['title' => 'Accept Visa Delta/Debit', 'desc' => 'Do you want to accept Visa Delta/Debit payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO' => ['title' => 'Accept Maestro', 'desc' => 'Do you want to accept Maestro payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_UKE' => ['title' => 'Accept Visa Electron UK Debit', 'desc' => 'Do you want to accept Visa Electron UK Debit payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_AMEX' => ['title' => 'Accept American Express', 'desc' => 'Do you want to accept American Express payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_DC' => ['title' => 'Accept Diners Club', 'desc' => 'Do you want to accept Diners Club payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_JCB' => ['title' => 'Accept Japan Credit Bureau', 'desc' => 'Do you want to accept Japan Credit Bureau payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_LASER' => ['title' => 'Accept Laser Card', 'desc' => 'Do you want to accept Laser Card payments?', 'value' => 'True', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), '], 'MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_PAYPAL' => ['title' => 'Accept PayPal', 'desc' => 'Do you want to accept PayPal payments?', 'value' => 'False', 'set_func' => 'tep_cfg_select_option(array(\'True\', \'False\'), ']];
    }
    public function send_transaction_to_gateway($url, $parameters): bool|string
    {
        ...
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_SSL == 'True') {
            ...
            if (is_file(OSCOM::get_config('dir_root', 'Shop') . 'ext/modules/payment/sage_pay/sagepay.com.crt')) {
                curl_setopt($curl, CURLOPT_CAINFO, OSCOM::get_config('dir_root', 'Shop') . 'ext/modules/payment/sage_pay/sagepay.com.crt');
            } elseif (is_file(OSCOM::get_config('dir_root', 'Shop') . 'includes/cacert.pem')) {
                curl_setopt($curl, CURLOPT_CAINFO, OSCOM::get_config('dir_root', 'Shop') . 'includes/cacert.pem');
            }
        } else {
    ...
    public function is_card($key): bool
    {
        if (!isset($this->_cards)) {
            $this->get_card_types();
        }
    ...
    public function delete_card($token, $token_id): bool
    {
        $OSCOM_Db = Registry::get('Db');
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_SERVER == 'Live') {
        ...
        $params = ['VPSProtocol' => $this->api_version, 'TxType' => 'REMOVETOKEN', 'Vendor' => substr(MODULE_PAYMENT_SAGE_PAY_DIRECT_VENDOR_LOGIN_NAME, 0, 15), 'Token' => $token];
        $post_string = '';
        foreach ($params as $key => $value) {
            $post_string .= $key . '=' . urlencode(trim($value)) . '&';
        }
        $response = $this->send_transaction_to_gateway($gateway_url, $post_string);
        $string_array = explode(chr(10), (string) $response);
        $sage_pay_response = [];
        foreach ($string_array as $string) {
            if (str_contains($string, '=')) {
                $parts = explode('=', $string, 2);
    ...
    public function load_error_messages(): void
    {
        $errors = [];
    ...
    public function get_error_message_number($string): string|false
    {
        if (!isset($this->_error_messages)) {
            $this->load_error_messages();
        }
        $error = explode(' ', (string) $string, 2);
        if (is_numeric($error[0]) && $this->error_message_number_exists($error[0])) {
            return $error[0];
        }
        return false;
    }
    public function get_error_message($number)
    {
        if (!isset($this->_error_messages)) {
            $this->load_error_messages();
        }
        if (is_numeric($number) && $this->error_message_number_exists($number)) {
            return $this->_error_messages[$number];
        }
        return false;
    }
    public function error_message_number_exists($number): bool
    {
        if (!isset($this->_error_messages)) {
            $this->load_error_messages();
        }
    ...
    public function get_test_link_info(): string
    {
        $dialog_title = OSCOM::get_def('module_payment_sage_pay_direct_dialog_connection_title');
        $dialog_button_close = OSCOM::get_def('module_payment_sage_pay_direct_dialog_connection_button_close');
        $dialog_success = OSCOM::get_def('module_payment_sage_pay_direct_dialog_connection_success');
        $dialog_failed = OSCOM::get_def('module_payment_sage_pay_direct_dialog_connection_failed');
        $dialog_error = OSCOM::get_def('module_payment_sage_pay_direct_dialog_connection_error');
        $dialog_connection_time = OSCOM::get_def('module_payment_sage_pay_direct_dialog_connection_time');
        ...
        EOD;
        $info = '<p><img src="images/icons/locked.gif" border="0">&nbsp;<a href="javascript:openTestConnectionDialog();" style="text-decoration: underline; font-weight: bold;">' . OSCOM::get_def('module_payment_sage_pay_direct_dialog_connection_link_title') . '</a></p>' . '<div id="testConnectionDialog" style="display: none;"><p>';
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_SERVER == 'Live') {
        ...
        return $info . ('</p><div id="testConnectionDialogProgress"><p>' . OSCOM::get_def('module_payment_sage_pay_direct_dialog_connection_general_text') . '</p><div id="tcdprogressbar"></div></div></div>' . $js);
    }
    public function get_test_connection_result(): int
    {
        if (MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_SERVER == 'Live') {
            $gateway_url = 'https://live.sagepay.com/gateway/service/vspdirect-register.vsp';
        } else {
        ...
        $params = ['VPSProtocol' => $this->api_version, 'ReferrerID' => 'C74D7B82-E9EB-4FBD-93DB-76F0F551C802', 'Vendor' => substr(MODULE_PAYMENT_SAGE_PAY_DIRECT_VENDOR_LOGIN_NAME, 0, 15), 'Amount' => 0, 'Currency' => DEFAULT_CURRENCY];
        $ip_address = HTTP::get_ip_address();
        if (!empty($ip_address) && ip2long($ip_address) != -1 && ip2long($ip_address) != false) {
            $params['ClientIPAddress'] = $ip_address;
        }
    ...
    public function template_class_exists(): bool
    {
        return class_exists('oscTemplate') && isset($GLOBALS['oscTemplate']) && is_object($GLOBALS['oscTemplate']) && $GLOBALS['oscTemplate']::class == 'oscTemplate';
    }
    public function get_submit_card_details_javascript(): string
    {
        return <<<EOD
        <script>
        if ( typeof jQuery == 'undefined' ) {
        ...
        if (tep_not_null(MODULE_PAYMENT_SAGE_PAY_DIRECT_DEBUG_EMAIL)) {
            ...
            if (!empty($_POST)) {
                if (isset($_POST['cc_number_nh-dns'])) {
                    $_POST['cc_number_nh-dns'] = 'XXXX' . substr((string) $_POST['cc_number_nh-dns'], -4);
                }
            ...
            if (!empty($email_body)) {
                $debug_email = new Mail(MODULE_PAYMENT_SAGE_PAY_DIRECT_DEBUG_EMAIL, null, STORE_OWNER_EMAIL_ADDRESS, STORE_OWNER, 'Sage Pay Direct Debug E-Mail');
                $debug_email->set_body($email_body);
                $debug_email->send();
            }
        }
    }
