...
class ar_tell_a_friend
{
    ...
    {
        $this->title = OSCOM::get_def('module_action_recorder_tell_a_friend_title');
        $this->description = OSCOM::get_def('module_action_recorder_tell_a_friend_description');
        if ($this->check()) {
    ...
    {
        $OSCOM_Db = Registry::get('Db');
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Minimum Minutes Per E-Mail', 'configuration_key' => 'MODULE_ACTION_RECORDER_TELL_A_FRIEND_EMAIL_MINUTES', 'configuration_value' => '15', 'configuration_description' => 'Minimum number of minutes to allow 1 e-mail to be sent (eg, 15 for 1 e-mail every 15 minutes)', 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()']);
    }
