...
class bm_best_sellers
{
    ...
    {
        $this->title = OSCOM::get_def('module_boxes_best_sellers_title');
        $this->description = OSCOM::get_def('module_boxes_best_sellers_description');
    ...
    {
        global $current_category_id, $osc_template;
        ...
        if (!isset($_GET['products_id'])) {
            ...
            $Qbest->bind_int(':language_id', $OSCOM_Language->get_id());
            if (isset($current_category_id) && $current_category_id > 0) {
                $Qbest->bind_int(':category_id', $current_category_id);
            }
            $Qbest->bind_int(':limit', MAX_DISPLAY_BESTSELLERS);
            $Qbest->execute();
            $best = $Qbest->fetch_all();
    ...
    {
        $OSCOM_Db = Registry::get('Db');
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Enable Best Sellers Module', 'configuration_key' => 'MODULE_BOXES_BEST_SELLERS_STATUS', 'configuration_value' => 'True', 'configuration_description' => 'Do you want to add the module to your shop?', 'configuration_group_id' => '6', 'sort_order' => '1', 'set_function' => 'tep_cfg_select_option(array(\'True\', \'False\'), ', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Content Placement', 'configuration_key' => 'MODULE_BOXES_BEST_SELLERS_CONTENT_PLACEMENT', 'configuration_value' => 'Right Column', 'configuration_description' => 'Should the module be loaded in the left or right column?', 'configuration_group_id' => '6', 'sort_order' => '1', 'set_function' => 'tep_cfg_select_option(array(\'Left Column\', \'Right Column\'), ', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Sort Order', 'configuration_key' => 'MODULE_BOXES_BEST_SELLERS_SORT_ORDER', 'configuration_value' => '0', 'configuration_description' => 'Sort order of display. Lowest is displayed first.', 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()']);
    }
    ...
    {
        return ['MODULE_BOXES_BEST_SELLERS_STATUS', 'MODULE_BOXES_BEST_SELLERS_CONTENT_PLACEMENT', 'MODULE_BOXES_BEST_SELLERS_SORT_ORDER'];
    }
}