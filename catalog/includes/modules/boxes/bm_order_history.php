...
class bm_order_history
{
    ...
    {
        $this->title = OSCOM::get_def('module_boxes_order_history_title');
        $this->description = OSCOM::get_def('module_boxes_order_history_description');
    ...
    public function execute(): void
    {
        ...
        if (isset($_SESSION['customer_id'])) {
            ...
            $Qorders->bind_int(':limit', MAX_DISPLAY_PRODUCTS_IN_ORDER_HISTORY_BOX);
            $Qorders->execute();
            if ($Qorders->fetch() !== false) {
                $product_ids = [];
                do {
                    $product_ids[] = $Qorders->value_int('products_id');
                } while ($Qorders->fetch());
                ...
                while ($Qproducts->fetch()) {
                    $customer_orders_string .= '<li><span class="pull-right"><a href="' . OSCOM::link(basename((string) $PHP_SELF), tep_get_all_get_params(['action']) . 'action=cust_order&pid=' . $Qproducts->value_int('products_id')) . '"><span class="fa fa-shopping-cart"></span></a></span><a href="' . OSCOM::link('product_info.php', 'products_id=' . $Qproducts->value_int('products_id')) . '">' . $Qproducts->value('products_name') . '</a></li>';
                }
                ob_start();
    ...
    {
        $OSCOM_Db = Registry::get('Db');
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Enable Order History Module', 'configuration_key' => 'MODULE_BOXES_ORDER_HISTORY_STATUS', 'configuration_value' => 'True', 'configuration_description' => 'Do you want to add the module to your shop?', 'configuration_group_id' => '6', 'sort_order' => '1', 'set_function' => 'tep_cfg_select_option(array(\'True\', \'False\'), ', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Content Placement', 'configuration_key' => 'MODULE_BOXES_ORDER_HISTORY_CONTENT_PLACEMENT', 'configuration_value' => 'Right Column', 'configuration_description' => 'Should the module be loaded in the left or right column?', 'configuration_group_id' => '6', 'sort_order' => '1', 'set_function' => 'tep_cfg_select_option(array(\'Left Column\', \'Right Column\'), ', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Sort Order', 'configuration_key' => 'MODULE_BOXES_ORDER_HISTORY_SORT_ORDER', 'configuration_value' => '0', 'configuration_description' => 'Sort order of display. Lowest is displayed first.', 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()']);
    }
