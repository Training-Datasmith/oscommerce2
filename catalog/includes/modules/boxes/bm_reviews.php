...
class bm_reviews
{
    ...
    {
        $this->title = OSCOM::get_def('module_boxes_reviews_title');
        $this->description = OSCOM::get_def('module_boxes_reviews_description');
    ...
    {
        global $currencies, $osc_template;
        ...
        $Qcheck->bind_int(':languages_id', $OSCOM_Language->get_id());
        if (isset($_GET['products_id'])) {
            $Qcheck->bind_int(':products_id', $_GET['products_id']);
        }
        $Qcheck->execute();
        $result = $Qcheck->fetch_all();
        if (count($result) > 0) {
            ...
            $Qreview->bind_int(':reviews_id', $result['reviews_id']);
            $Qreview->bind_int(':languages_id', $OSCOM_Language->get_id());
            $Qreview->execute();
            if ($Qreview->fetch() !== false) {
                // display random review box
                $rand_review_text = tep_break_string($Qreview->value_protected('reviews_text'), 15, '-<br />');
                $reviews_box_contents = '<div class="text-center"><a href="' . OSCOM::link('product_reviews.php', 'products_id=' . $Qreview->value_int('products_id')) . '">' . HTML::image(OSCOM::link_image($Qreview->value('products_image')), $Qreview->value('products_name'), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a></div><div><a href="' . OSCOM::link('product_reviews.php', 'products_id=' . $Qreview->value_int('products_id')) . '">' . $rand_review_text . '</a>...</div><div class="text-center" title="' . OSCOM::get_def('module_boxes_reviews_box_text_of_5_stars', ['reviews_rating' => $Qreview->value_int('reviews_rating')]) . '">' . HTML::stars($Qreview->value_int('reviews_rating')) . '</div>';
            }
        } elseif (isset($_GET['products_id'])) {
            // display 'write a review' box
            $reviews_box_contents = '<span class="fa fa-thumbs-up"></span> <a href="' . OSCOM::link('product_reviews_write.php', 'products_id=' . (int) $_GET['products_id']) . '">' . OSCOM::get_def('module_boxes_reviews_box_write_review') . '</a>';
        } else {
            // display 'no reviews' box
            $reviews_box_contents = '<p>' . OSCOM::get_def('module_boxes_reviews_box_no_reviews') . '</p>';
        }
        ob_start();
    ...
    {
        $OSCOM_Db = Registry::get('Db');
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Enable Reviews Module', 'configuration_key' => 'MODULE_BOXES_REVIEWS_STATUS', 'configuration_value' => 'True', 'configuration_description' => 'Do you want to add the module to your shop?', 'configuration_group_id' => '6', 'sort_order' => '1', 'set_function' => 'tep_cfg_select_option(array(\'True\', \'False\'), ', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Content Placement', 'configuration_key' => 'MODULE_BOXES_REVIEWS_CONTENT_PLACEMENT', 'configuration_value' => 'Right Column', 'configuration_description' => 'Should the module be loaded in the left or right column?', 'configuration_group_id' => '6', 'sort_order' => '1', 'set_function' => 'tep_cfg_select_option(array(\'Left Column\', \'Right Column\'), ', 'date_added' => 'now()']);
        $OSCOM_Db->save('configuration', ['configuration_title' => 'Sort Order', 'configuration_key' => 'MODULE_BOXES_REVIEWS_SORT_ORDER', 'configuration_value' => '0', 'configuration_description' => 'Sort order of display. Lowest is displayed first.', 'configuration_group_id' => '6', 'sort_order' => '0', 'date_added' => 'now()']);
    }
