...
$OSCOM_Language->load_definitions('modules/content/account/cm_account_set_password');
if (isset($_POST['action']) && $_POST['action'] == 'process' && isset($_POST['formid']) && $_POST['formid'] == $_SESSION['sessiontoken']) {
    ...
    if ($error == false) {
        ...
        $message_stack->add_session('account', MODULE_CONTENT_ACCOUNT_SET_PASSWORD_SUCCESS_PASSWORD_SET, 'success');
        OSCOM::redirect('account.php');
    }
}
...
<?php 
echo HTML::form('account_password', OSCOM::link('ext/modules/content/account/set_password.php'), 'post', 'class="form-horizontal"', ['tokenize' => true, 'action' => 'process']);
?>
...
?></label>
      <div class="col-sm-9">
        <?php 
echo HTML::password_field('password_new', null, 'required aria-required="true" autofocus="autofocus" id="inputPassword" autocomplete="new-password" placeholder="' . OSCOM::get_def('entry_password_new_text') . '"');
