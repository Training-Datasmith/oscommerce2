...
$OSCOM_Language->load_definitions('modules/payment/sage_pay_server');
    ...
    if ($Qsp->fetch() !== false) {
        ...
        if ($_POST['VPSSignature'] == $sig) {
            if ($_POST['Status'] == 'OK' || $_POST['Status'] == 'AUTHENTICATED' || $_POST['Status'] == 'REGISTERED') {
                ...
                $result = 'Status=OK' . chr(13) . chr(10) . 'RedirectURL=' . $sage_pay_server->format_url(OSCOM::link('checkout_process.php', 'check=PROCESS&skcode=' . $skcode, false));
            } else {
                $error = isset($_POST['StatusDetail']) ? $sage_pay_server->get_error_message_number($_POST['StatusDetail']) : null;
                ...
                $result = 'Status=OK' . chr(13) . chr(10) . 'RedirectURL=' . $sage_pay_server->format_url($error_url);
                $OSCOM_Db->delete('sagepay_server_securitykeys', ['code' => $skcode]);
                $sage_pay_server->send_debug_email();
            }
        } else {
            $result = 'Status=INVALID' . chr(13) . chr(10) . 'RedirectURL=' . $sage_pay_server->format_url(OSCOM::link('shopping_cart.php', '', false));
            $sage_pay_server->send_debug_email();
        }
    }
}
if (!isset($result)) {
    $result = 'Status=ERROR' . chr(13) . chr(10) . 'RedirectURL=' . $sage_pay_server->format_url(OSCOM::link('shopping_cart.php', '', false));
}
echo $result;
