...
switch ($_GET['action']) {
    ...
    case 'manufacturer':
        if (isset($_GET['manufacturers_id']) && is_numeric($_GET['manufacturers_id'])) {
            $Qmanufacturer = $OSCOM_Db->get('manufacturers_info', 'manufacturers_url', ['manufacturers_id' => $_GET['manufacturers_id'], 'languages_id' => $OSCOM_Language->get_id()]);
            if ($Qmanufacturer->fetch() !== false) {
                ...
                if (!empty($Qmanufacturer->value('manufacturers_url'))) {
                    ...
                    $Qupdate->bind_int(':manufacturers_id', $_GET['manufacturers_id']);
                    $Qupdate->bind_int(':languages_id', $OSCOM_Language->get_id());
                    $Qupdate->execute();
                ...
                $Qmanufacturer->bind_value(':default_language', DEFAULT_LANGUAGE);
                $Qmanufacturer->execute();
                if ($Qmanufacturer->fetch() !== false) {
                    if (!empty($Qmanufacturer->value('manufacturers_url'))) {
                        $Qupdate = $OSCOM_Db->prepare('update :table_manufacturers_info set url_clicked = url_clicked+1, date_last_click = now() where manufacturers_id = :manufacturers_id and languages_id = :languages_id');
                        $Qupdate->bind_int(':manufacturers_id', $_GET['manufacturers_id']);
                        $Qupdate->bind_int(':languages_id', $Qmanufacturer->value_int('languages_id'));
                        $Qupdate->execute();
                        HTTP::redirect($Qmanufacturer->value('manufacturers_url'));
                    }
                }
