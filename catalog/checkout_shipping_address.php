...
$OSCOM_Language->load_definitions('checkout_shipping_address');
    ...
    if (tep_not_null($_POST['firstname']) && tep_not_null($_POST['lastname']) && tep_not_null($_POST['street_address'])) {
        ...
        if (ACCOUNT_GENDER == 'true') {
            if ($gender != 'm' && $gender != 'f') {
                $error = true;
                $message_stack->add('checkout_address', OSCOM::get_def('entry_gender_error'));
            }
        }
        if (strlen((string) $firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
            $error = true;
            $message_stack->add('checkout_address', OSCOM::get_def('entry_first_name_error', ['min_length' => ENTRY_FIRST_NAME_MIN_LENGTH]));
        }
        if (strlen((string) $lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
            $error = true;
            $message_stack->add('checkout_address', OSCOM::get_def('entry_last_name_error', ['min_length' => ENTRY_LAST_NAME_MIN_LENGTH]));
        }
        if (strlen($street_address) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
            $error = true;
            $message_stack->add('checkout_address', OSCOM::get_def('entry_street_address_error', ['min_length' => ENTRY_STREET_ADDRESS_MIN_LENGTH]));
        }
        if (strlen((string) $postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
            $error = true;
            $message_stack->add('checkout_address', OSCOM::get_def('entry_post_code_error', ['min_length' => ENTRY_POSTCODE_MIN_LENGTH]));
        }
        if (strlen((string) $city) < ENTRY_CITY_MIN_LENGTH) {
            $error = true;
            $message_stack->add('checkout_address', OSCOM::get_def('entry_city_error', ['min_length' => ENTRY_CITY_MIN_LENGTH]));
        }
        if (ACCOUNT_STATE == 'true') {
            $zone_id = 0;
            ...
            if ($entry_state_has_zones == true) {
                ...
                if (count($Qzone->fetch_all()) === 1) {
                    ...
                    $error = true;
                    $message_stack->add('checkout_address', OSCOM::get_def('entry_state_error_select'));
                }
            } else if (strlen((string) $state) < ENTRY_STATE_MIN_LENGTH) {
                $error = true;
                $message_stack->add('checkout_address', OSCOM::get_def('entry_state_error', ['min_length' => ENTRY_STATE_MIN_LENGTH]));
            }
        }
        if (is_numeric($country) == false || $country < 1) {
            $error = true;
            $message_stack->add('checkout_address', OSCOM::get_def('entry_country_error'));
        }
        if ($error == false) {
            ...
            $_SESSION['sendto'] = $OSCOM_Db->last_insert_id();
        ...
        $Qcheck->bind_int(':address_book_id', $_SESSION['sendto']);
...
$breadcrumb->add(OSCOM::get_def('navbar_title_1'), OSCOM::link('checkout_shipping.php'));
$breadcrumb->add(OSCOM::get_def('navbar_title_2'), OSCOM::link('checkout_shipping_address.php'));
...
<?php 
if ($message_stack->size('checkout_address') > 0) {
    echo $message_stack->output('checkout_address');
}
?>

<?php 
echo HTML::form('checkout_address', OSCOM::link('checkout_shipping_address.php'), 'post', 'class="form-horizontal"', ['tokenize' => true]);
?>
...
if ($process == false) {
  ...
  <h2><?php 
    echo OSCOM::get_def('table_heading_shipping_address');
    ?></h2>

  <div class="contentText row">
    <div class="col-sm-8">
      <div class="alert alert-warning"><?php 
    echo OSCOM::get_def('text_selected_shipping_destination');
    ?></div>
    </div>
    <div class="col-sm-4">
      <div class="panel panel-primary">
        <div class="panel-heading"><?php 
    echo OSCOM::get_def('title_shipping_address');
    ?></div>
...
<?php 
  ...
  <h2><?php 
        echo OSCOM::get_def('table_heading_address_book_entries');
        ?></h2>

  <div class="alert alert-info"><?php 
        echo OSCOM::get_def('text_select_other_shipping_destination');
        ?></div>
...
<?php 
        ...
        $Qab->bind_int(':customers_id', $_SESSION['customer_id']);
        $Qab->execute();
        while ($Qab->fetch()) {
            $format_id = tep_get_address_format_id($Qab->value_int('country_id'));
            ?>

      <div class="col-sm-4">
        <div class="panel panel-<?php 
            echo $Qab->value_int('address_book_id') == $_SESSION['sendto'] ? 'primary' : 'default';
            ?>">
          <div class="panel-heading"><?php 
            echo HTML::output_protected($Qab->value('firstname') . ' ' . $Qab->value('lastname'));
            ?></strong></div>
          <div class="panel-body">
            <?php 
            echo tep_address_format($format_id, $Qab->to_array(), true, ' ', '<br />');
            ?>
          </div>
          <div class="panel-footer text-center"><?php 
            echo HTML::radio_field('address', $Qab->value_int('address_book_id'), $Qab->value_int('address_book_id') == $_SESSION['sendto']);
            ?></div>
        </div>
      </div>
...
if ($addresses_count < MAX_ADDRESS_BOOK_ENTRIES) {
  ...
  <h2><?php 
    echo OSCOM::get_def('table_heading_new_shipping_address');
    ?></h2>

  <div class="alert alert-info"><?php 
    echo OSCOM::get_def('text_create_new_shipping_address');
    ?></div>
...
echo HTML::hidden_field('action', 'submit') . HTML::button(OSCOM::get_def('image_button_continue'), 'fa fa-angle-right', null, null, 'btn-success');
?></div>
  </div>
    ...
    <div class="stepwizard">
      <div class="stepwizard-row">
        <div class="stepwizard-step">
          ...
          <p><?php 
echo OSCOM::get_def('checkout_bar_delivery');
?></p>
        </div>
        <div class="stepwizard-step">
          <button type="button" class="btn btn-default btn-circle" disabled="disabled">2</button>
          <p><?php 
echo OSCOM::get_def('checkout_bar_payment');
?></p>
        </div>
        <div class="stepwizard-step">
          <button type="button" class="btn btn-default btn-circle" disabled="disabled">3</button>
          <p><?php 
echo OSCOM::get_def('checkout_bar_confirmation');
?></p>
        </div>
      </div>
    </div>
...
if ($process == true) {
  ...
  <div class="buttonSet">
    <?php 
    echo HTML::button(OSCOM::get_def('image_button_back'), 'fa fa-angle-left', OSCOM::link('checkout_shipping_address.php'));
    ?>
  </div>
