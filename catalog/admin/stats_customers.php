...
echo OSCOM::get_def('table_heading_total_purchased');
?>&nbsp;</td>
              </tr>
<?php 
$rows = 0;
...
while ($Qcustomers->fetch()) {
    ...
    echo OSCOM::link(FILENAME_CUSTOMERS, 'search=' . $Qcustomers->value('customers_lastname'));
    ?>'">
                <td class="dataTableContent"><?php 
    echo $rows;
    ?>.</td>
                <td class="dataTableContent"><?php 
    echo '<a href="' . OSCOM::link(FILENAME_CUSTOMERS, 'search=' . $Qcustomers->value('customers_lastname')) . '">' . $Qcustomers->value('customers_firstname') . ' ' . $Qcustomers->value('customers_lastname') . '</a>';
    ?></td>
                <td class="dataTableContent" align="right"><?php 
    echo $currencies->format($Qcustomers->value('ordersum'));
    ?>&nbsp;</td>
              </tr>
<?php 
}
