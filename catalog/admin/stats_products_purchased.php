...
echo OSCOM::get_def('table_heading_purchased');
?>&nbsp;</td>
              </tr>
<?php 
$rows = 0;
...
while ($Qproducts->fetch()) {
    ...
    echo OSCOM::link(FILENAME_CATEGORIES, 'action=new_product_preview&read=only&pID=' . $Qproducts->value_int('products_id') . '&origin=' . FILENAME_STATS_PRODUCTS_PURCHASED . '&page=' . $_GET['page']);
    ?>'">
                <td class="dataTableContent"><?php 
    echo $rows;
    ?>.</td>
                <td class="dataTableContent"><?php 
    echo '<a href="' . OSCOM::link(FILENAME_CATEGORIES, 'action=new_product_preview&read=only&pID=' . $Qproducts->value_int('products_id') . '&origin=' . FILENAME_STATS_PRODUCTS_PURCHASED . '&page=' . $_GET['page']) . '">' . $Qproducts->value('products_name') . '</a>';
    ?></td>
                <td class="dataTableContent" align="center"><?php 
    echo $Qproducts->value_int('products_ordered');
    ?>&nbsp;</td>
              </tr>
<?php 
}
