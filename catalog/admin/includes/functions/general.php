...
function tep_get_category_tree($parent_id = '0', string $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false)
{
    ...
    if ((sizeof($category_tree_array) < 1) && ($exclude != '0')) {
        $category_tree_array[] = ['id' => '0', 'text' => OSCOM::getDef('text_top')];
    }
    ...
    while ($Qcategories->fetch()) {
        if ($exclude != $Qcategories->valueInt('categories_id')) {
            $category_tree_array[] = ['id' => $Qcategories->valueInt('categories_id'), 'text' => $spacing . $Qcategories->value('categories_name')];
        }
