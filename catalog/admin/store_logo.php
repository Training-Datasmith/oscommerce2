...
if (tep_not_null($action)) {
    switch ($action) {
        case 'save':
            ...
            $store_logo->set_destination(OSCOM::get_config('dir_root', 'Shop') . 'images/');
            if ($store_logo->parse()) {
                if ($store_logo->save()) {
                    $OSCOM_Db->save('configuration', ['configuration_value' => $store_logo->filename, 'last_modified' => 'now()'], ['configuration_key' => 'STORE_LOGO']);
                    $oscom_message_stack->add(OSCOM::get_def('success_logo_updated'), 'success');
                } else {
                    $error = true;
                }
            } else {
...
if (!File_System::is_writable(OSCOM::get_config('dir_root', 'Shop') . 'images/')) {
    $oscom_message_stack->add(OSCOM::get_def('error_images_directory_not_writeable', ['sec_dir_permissions_link' => OSCOM::link(FILENAME_SEC_DIR_PERMISSIONS)]), 'error');
}
...
?></td>
      ...
      <tr>
        <td><?php 
echo HTML::image(OSCOM::link_image('Shop/' . STORE_LOGO));
?></td>
      </tr>
      <tr>
        <td><?php 
...
echo OSCOM::get_def('text_logo_image');
?></td>
              <td class="main"><?php 
echo HTML::file_field('store_logo');
?></td>
...
?></td>
      ...
      <tr>
        <td class="main"><?php 
echo OSCOM::get_def('text_format_and_location');
?></td>
      </tr>
      <tr>
        <td class="main"><?php 
echo OSCOM::get_config('dir_root', 'Shop') . 'images/' . STORE_LOGO;
?></td>
      </tr>
    </table>
