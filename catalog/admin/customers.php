      ...
      while ($Qcustomers->fetch()) {
          ...
          ?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($Qcustomers->valueInt('customers_id') === (int)$cInfo->customers_id)) {
                    ...
                    echo '<a href="' . OSCOM::link(FILENAME_CUSTOMERS, tep_get_all_get_params(['cID']) . 'cID=' . $Qcustomers->valueInt('customers_id')) . '">' . HTML::image(OSCOM::linkImage('icon_info.gif'), OSCOM::getDef('image_icon_info')) . '</a>';
                } ?>&nbsp;</td>
              </tr>
<?php
      }
      ...
      switch ($action) {
          case 'confirm':
              ...
              if (isset($cInfo->number_of_reviews) && ($cInfo->number_of_reviews) > 0) {
                  $contents[] = ['text' => '<br />' . HTML::checkboxField('delete_reviews', 'on', true) . ' ' . OSCOM::getDef('text_delete_reviews', ['number_of_reviews' => $cInfo->number_of_reviews])];
              }
