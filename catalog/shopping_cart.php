...
$OSCOM_Language->load_definitions('shopping_cart');
$breadcrumb->add(OSCOM::get_def('navbar_title'), OSCOM::link('shopping_cart.php'));
...
<?php 
    echo HTML::form('cart_quantity', OSCOM::link('shopping_cart.php', 'action=update_product'));
    ?>
    ...
    for ($i = 0, $n = sizeof($products); $i < $n; $i++) {
        ...
        if (isset($products[$i]['attributes']) && is_array($products[$i]['attributes'])) {
            foreach ($products[$i]['attributes'] as $option => $value) {
                echo HTML::hidden_field('id[' . $products[$i]['id'] . '][' . $option . ']', $value);
...
<?php 
    ...
    for ($i = 0, $n = sizeof($products); $i < $n; $i++) {
        ...
        $products_name .= '<br>' . HTML::input_field('cart_quantity[]', $products[$i]['quantity'], 'style="width: 65px;" min="0"', 'number') . HTML::hidden_field('products_id[]', $products[$i]['id']) . ' ' . HTML::button(OSCOM::get_def('text_cart_button_update'), 'fa fa-refresh', null, null, 'btn-info btn-xs') . ' ' . HTML::button(OSCOM::get_def('text_cart_button_remove'), 'fa fa-remove', OSCOM::link('shopping_cart.php', 'products_id=' . $products[$i]['id'] . '&action=remove_product'), null, 'btn-danger btn-xs');
        $products_name .= '</td>';
    ...
    echo OSCOM::get_def('sub_title_sub_total');
    ?> <?php 
...
<?php 
    ...
    <div class="alert alert-warning"><?php 
            echo OSCOM::get_def('out_of_stock_can_checkout', ['out_of_stock_label' => STOCK_MARK_PRODUCT_OUT_OF_STOCK]);
            ?></div>
...
<?php 
    ...
    <div class="alert alert-danger"><?php 
            echo OSCOM::get_def('out_of_stock_cant_checkout', ['out_of_stock_label' => STOCK_MARK_PRODUCT_OUT_OF_STOCK]);
            ?></div>
...
<?php 
  ...
  <div class="buttonSet">
    ...
    echo HTML::button(OSCOM::get_def('image_button_checkout'), 'fa fa-angle-right', OSCOM::link('checkout_shipping.php'), null, 'btn-success');
    ?></div>
  </div>
...
<?php 
    ...
    if (!empty($checkout_buttons)) {
        ...
        echo '<p class="text-right">' . OSCOM::get_def('text_alternative_checkout_methods') . '</p>';
...
<div class="alert alert-danger">
  <?php 
    echo OSCOM::get_def('text_cart_empty');
    ?>
</div>

<p class="text-right"><?php 
    echo HTML::button(OSCOM::get_def('image_button_continue'), 'fa fa-angle-right', OSCOM::link('index.php'), null, 'btn-danger');
    ?></p>
