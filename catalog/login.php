...
$page_content = $osc_template->get_content('login');
if (is_int($login_customer_id) && $login_customer_id > 0) {
    ...
    $Qcustomer->bind_int(':customers_id', $login_customer_id);
    $Qcustomer->execute();
    $_SESSION['customer_id'] = $login_customer_id;
    $_SESSION['customer_default_address_id'] = $Qcustomer->value_int('customers_default_address_id');
    $_SESSION['customer_first_name'] = $Qcustomer->value('customers_firstname');
    $_SESSION['customer_country_id'] = $Qcustomer->value_int('entry_country_id');
    $_SESSION['customer_zone_id'] = $Qcustomer->value_int('entry_zone_id');
    ...
    $Qupdate->bind_int(':customers_info_id', $_SESSION['customer_id']);
    $Qupdate->execute();
...
$OSCOM_Language->load_definitions('login');
$breadcrumb->add(OSCOM::get_def('navbar_title'), OSCOM::link('login.php'));
