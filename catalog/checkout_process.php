...
if (isset($_SESSION['cart']->cart_id) && isset($_SESSION['cartID'])) {
    if ($_SESSION['cart']->cart_id != $_SESSION['cartID']) {
        OSCOM::redirect('checkout_shipping.php');
    }
}
$OSCOM_Language->load_definitions('checkout_process');
...
if (!isset($OSCOM_PM) || $payment_modules->selected_module != $_SESSION['payment'] || $OSCOM_PM->enabled == false) {
    OSCOM::redirect('checkout_payment.php', 'error_message=' . urlencode((string) OSCOM::get_def('error_no_payment_module_selected')));
}
...
$sql_data_array = ['orders_id' => $insert_id, 'orders_status_id' => $order->info['order_status'], 'date_added' => 'now()', 'customer_notified' => $customer_notification, 'comments' => $order->info['comments']];
...
for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
    ...
    if (STOCK_LIMITED == 'true') {
        if (DOWNLOAD_ENABLED == 'true') {
            ...
            $Qstock->bind_int(':products_id', tep_get_prid($order->products[$i]['id']));
            if (is_array($products_attributes)) {
                $Qstock->bind_int(':options_id', $products_attributes[0]['option_id']);
                $Qstock->bind_int(':options_values_id', $products_attributes[0]['value_id']);
            }
            $Qstock->execute();
        } else {
        ...
        if ($Qstock->fetch() !== false) {
            ...
            if (DOWNLOAD_ENABLED != 'true' || tep_not_null($Qstock->value('products_attributes_filename'))) {
                $stock_left = $Qstock->value_int('products_quantity') - $order->products[$i]['qty'];
            } else {
                $stock_left = $Qstock->value_int('products_quantity');
            }
            if ($stock_left != $Qstock->value_int('products_quantity')) {
    ...
    $Qupdate->bind_int(':products_ordered', $order->products[$i]['qty']);
    $Qupdate->bind_int(':products_id', tep_get_prid($order->products[$i]['id']));
    $Qupdate->execute();
    $sql_data_array = ['orders_id' => $insert_id, 'products_id' => tep_get_prid($order->products[$i]['id']), 'products_model' => $order->products[$i]['model'], 'products_name' => $order->products[$i]['name'], 'products_price' => $order->products[$i]['price'], 'final_price' => $order->products[$i]['final_price'], 'products_tax' => $order->products[$i]['tax'], 'products_quantity' => $order->products[$i]['qty']];
    $OSCOM_Db->save('orders_products', $sql_data_array);
    $order_products_id = $OSCOM_Db->last_insert_id();
    ...
    if (isset($order->products[$i]['attributes'])) {
        ...
        for ($j = 0, $n2 = sizeof($order->products[$i]['attributes']); $j < $n2; $j++) {
            ...
            $Qattributes->bind_int(':products_id', $order->products[$i]['id']);
            $Qattributes->bind_int(':options_id', $order->products[$i]['attributes'][$j]['option_id']);
            $Qattributes->bind_int(':options_values_id', $order->products[$i]['attributes'][$j]['value_id']);
            $Qattributes->bind_int(':language_id', $OSCOM_Language->get_id());
            $Qattributes->execute();
            $sql_data_array = ['orders_id' => $insert_id, 'orders_products_id' => $order_products_id, 'products_options' => $Qattributes->value('products_options_name'), 'products_options_values' => $Qattributes->value('products_options_values_name'), 'options_values_price' => $Qattributes->value('options_values_price'), 'price_prefix' => $Qattributes->value('price_prefix')];
            $OSCOM_Db->save('orders_products_attributes', $sql_data_array);
            if (DOWNLOAD_ENABLED == 'true' && $Qattributes->has_value('products_attributes_filename') && tep_not_null($Qattributes->value('products_attributes_filename'))) {
                $sql_data_array = ['orders_id' => $insert_id, 'orders_products_id' => $order_products_id, 'orders_products_filename' => $Qattributes->value('products_attributes_filename'), 'download_maxdays' => $Qattributes->value('products_attributes_maxdays'), 'download_count' => $Qattributes->value('products_attributes_maxcount')];
...
$email_order = STORE_NAME . "\n" . OSCOM::get_def('email_separator') . "\n" . OSCOM::get_def('email_text_order_number') . ' ' . $insert_id . "\n" . OSCOM::get_def('email_text_invoice_url') . ' ' . OSCOM::link('account_history_info.php', 'order_id=' . $insert_id, false) . "\n" . OSCOM::get_def('email_text_date_ordered') . ' ' . strftime(OSCOM::get_def('date_format_long')) . "\n\n";
if ($order->info['comments']) {
    $email_order .= HTML::output_protected($order->info['comments']) . "\n\n";
}
$email_order .= OSCOM::get_def('email_text_products') . "\n" . OSCOM::get_def('email_separator') . "\n" . $products_ordered . OSCOM::get_def('email_separator') . "\n";
for ($i = 0, $n = sizeof($order_totals); $i < $n; $i++) {
    $email_order .= strip_tags((string) $order_totals[$i]['title']) . ' ' . strip_tags((string) $order_totals[$i]['text']) . "\n";
}
if ($order->content_type != 'virtual') {
    $email_order .= "\n" . OSCOM::get_def('email_text_delivery_address') . "\n" . OSCOM::get_def('email_separator') . "\n" . tep_address_label($_SESSION['customer_id'], $_SESSION['sendto'], 0, '', "\n") . "\n";
}
$email_order .= "\n" . OSCOM::get_def('email_text_billing_address') . "\n" . OSCOM::get_def('email_separator') . "\n" . tep_address_label($_SESSION['customer_id'], $_SESSION['billto'], 0, '', "\n") . "\n\n";
if (isset($OSCOM_PM)) {
    $email_order .= OSCOM::get_def('email_text_payment_method') . "\n" . OSCOM::get_def('email_separator') . "\n";
    $email_order .= $order->info['payment_method'] . "\n\n";
    if (isset($OSCOM_PM->email_footer)) {
        $email_order .= $OSCOM_PM->email_footer . "\n\n";
    }
}
$order_email = new Mail($order->customer['email_address'], $order->customer['firstname'] . ' ' . $order->customer['lastname'], STORE_OWNER_EMAIL_ADDRESS, STORE_OWNER, OSCOM::get_def('email_text_subject'));
$order_email->set_body($email_order);
$order_email->send();
// send emails to other people
if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
    $extra_email = new Mail(SEND_EXTRA_ORDER_EMAILS_TO, null, STORE_OWNER_EMAIL_ADDRESS, STORE_OWNER, OSCOM::get_def('email_text_subject'));
    $extra_email->set_body($email_order);
    $extra_email->send();
}
