...
$OSCOM_Language->load_definitions('account_edit');
if (isset($_POST['action']) && $_POST['action'] == 'process' && isset($_POST['formid']) && $_POST['formid'] == $_SESSION['sessiontoken']) {
    ...
    if (ACCOUNT_GENDER == 'true') {
        if ($gender != 'm' && $gender != 'f') {
            $error = true;
            $message_stack->add('account_edit', OSCOM::get_def('entry_gender_error'));
        }
    }
    if (strlen((string) $firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
        $error = true;
        $message_stack->add('account_edit', OSCOM::get_def('entry_first_name_error', ['min_length' => ENTRY_FIRST_NAME_MIN_LENGTH]));
    }
    if (strlen((string) $lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
        $error = true;
        $message_stack->add('account_edit', OSCOM::get_def('entry_last_name_error', ['min_length' => ENTRY_LAST_NAME_MIN_LENGTH]));
    }
    if (ACCOUNT_DOB == 'true') {
        $dob_date_time = new DateTime($dob);
        if (strlen((string) $dob) < ENTRY_DOB_MIN_LENGTH || $dob_date_time->is_valid() === false) {
            $error = true;
            $message_stack->add('account_edit', OSCOM::get_def('entry_date_of_birth_error'));
        }
    }
    if (strlen($email_address) < ENTRY_EMAIL_ADDRESS_MIN_LENGTH) {
        $error = true;
        $message_stack->add('account_edit', OSCOM::get_def('entry_email_address_error', ['min_length' => ENTRY_EMAIL_ADDRESS_MIN_LENGTH]));
    }
    if (!Is::email($email_address)) {
        $error = true;
        $message_stack->add('account_edit', OSCOM::get_def('entry_email_address_check_error'));
    }
    ...
    if ($Qcheck->fetch() !== false) {
        $error = true;
        $message_stack->add('account_edit', OSCOM::get_def('entry_email_address_error_exists'));
    }
    if (strlen((string) $telephone) < ENTRY_TELEPHONE_MIN_LENGTH) {
        $error = true;
        $message_stack->add('account_edit', OSCOM::get_def('entry_telephone_number_error', ['min_length' => ENTRY_TELEPHONE_MIN_LENGTH]));
    }
    if ($error == false) {
        $sql_data_array = ['customers_firstname' => $firstname, 'customers_lastname' => $lastname, 'customers_email_address' => $email_address, 'customers_telephone' => $telephone, 'customers_fax' => $fax];
        if (ACCOUNT_GENDER == 'true') {
        ...
        $message_stack->add_session('account', OSCOM::get_def('success_account_updated'), 'success');
        OSCOM::redirect('account.php');
    }
}
$Qaccount = $OSCOM_Db->prepare('select * from :table_customers where customers_id = :customers_id');
$Qaccount->bind_int(':customers_id', $_SESSION['customer_id']);
$Qaccount->execute();
$breadcrumb->add(OSCOM::get_def('navbar_title_1'), OSCOM::link('account.php'));
$breadcrumb->add(OSCOM::get_def('navbar_title_2'), OSCOM::link('account_edit.php'));
...
<?php 
if ($message_stack->size('account_edit') > 0) {
    echo $message_stack->output('account_edit');
}
?>

<?php 
echo HTML::form('account_edit', OSCOM::link('account_edit.php'), 'post', 'class="form-horizontal"', ['tokenize' => true, 'action' => 'process']);
?>
  ...
  <div class="form-group has-feedback">
    ...
    <div class="col-sm-9">
      <label class="radio-inline">
        <?php 
    echo HTML::radio_field('gender', 'm', $male, 'required aria-required="true" aria-describedby="atGender"') . ' ' . OSCOM::get_def('male');
    ?>
      </label>
      <label class="radio-inline">
        <?php 
...
?></label>
    <div class="col-sm-9">
      <?php 
echo HTML::input_field('firstname', $Qaccount->value('customers_firstname'), 'required aria-required="true" id="inputFirstName" placeholder="' . OSCOM::get_def('entry_first_name_text') . '"');
?>
      <?php 
echo OSCOM::get_def('form_required_input');
?>
...
?></label>
    <div class="col-sm-9">
      <?php 
echo HTML::input_field('lastname', $Qaccount->value('customers_lastname'), 'required aria-required="true" id="inputLastName" placeholder="' . OSCOM::get_def('entry_last_name_text') . '"');
?>
      <?php 
echo OSCOM::get_def('form_required_input');
?>
...
if (ACCOUNT_DOB == 'true') {
  ...
  <div class="form-group has-feedback">
    ...
    <div class="col-sm-9">
      <?php 
    echo HTML::input_field('dob', DateTime::to_short($Qaccount->value('customers_dob')), 'data-provide="datepicker" required aria-required="true" id="dob" placeholder="' . OSCOM::get_def('entry_date_of_birth_text') . '"');
    ?>
      <?php 
    echo OSCOM::get_def('form_required_input');
    ?>
...
?></label>
    <div class="col-sm-9">
      <?php 
echo HTML::input_field('email_address', $Qaccount->value('customers_email_address'), 'required aria-required="true" id="inputEmail" placeholder="' . OSCOM::get_def('entry_email_address_text') . '"', 'email');
?>
      <?php 
echo OSCOM::get_def('form_required_input');
?>
...
?></label>
    <div class="col-sm-9">
      <?php 
echo HTML::input_field('telephone', $Qaccount->value('customers_telephone'), 'required aria-required="true" id="inputTelephone" placeholder="' . OSCOM::get_def('entry_telephone_number_text') . '"', 'tel');
?>
      <?php 
echo OSCOM::get_def('form_required_input');
?>
...
?></label>
    <div class="col-sm-9">
      <?php 
echo HTML::input_field('fax', $Qaccount->value('customers_fax'), 'id="inputFax" placeholder="' . OSCOM::get_def('entry_fax_number_text') . '"');
?>
    </div>
  </div>
