...
$error = false;
if (isset($_GET['keywords']) && empty($_GET['keywords']) && (isset($_GET['dfrom']) && (empty($_GET['dfrom']) || $_GET['dfrom'] == OSCOM::get_def('dob_format_string'))) && (isset($_GET['dto']) && (empty($_GET['dto']) || $_GET['dto'] == OSCOM::get_def('dob_format_string'))) && (isset($_GET['pfrom']) && !is_numeric($_GET['pfrom'])) && (isset($_GET['pto']) && !is_numeric($_GET['pto']))) {
    $error = true;
    $message_stack->add_session('search', OSCOM::get_def('error_at_least_one_input'));
} else {
    $dfrom = '';
    $dto = '';
    $pfrom = '';
    ...
    $keywords = '';
    if (isset($_GET['dfrom'])) {
        $dfrom = $_GET['dfrom'] == OSCOM::get_def('dob_format_string') ? '' : $_GET['dfrom'];
    }
    if (isset($_GET['dto'])) {
        $dto = $_GET['dto'] == OSCOM::get_def('dob_format_string') ? '' : $_GET['dto'];
    }
    if (isset($_GET['pfrom'])) {
        $pfrom = $_GET['pfrom'];
    }
    ...
    if (tep_not_null($dfrom)) {
        $dfrom_date_time = new DateTime($dfrom);
        if ($dfrom_date_time->is_valid() === false) {
            $error = true;
            $date_check_error = true;
            $message_stack->add_session('search', OSCOM::get_def('error_invalid_from_date'));
        }
    }
    if (tep_not_null($dto)) {
        $dto_date_time = new DateTime($dto);
        if ($dto_date_time->is_valid() === false) {
            $error = true;
            $date_check_error = true;
            $message_stack->add_session('search', OSCOM::get_def('error_invalid_to_date'));
        }
    }
    if ($date_check_error == false && isset($dfrom_date_time) && $dfrom_date_time->is_valid() && isset($dto_date_time) && $dto_date_time->is_valid()) {
        if ($dfrom_date_time->get() > $dto_date_time->get()) {
            $error = true;
            $message_stack->add_session('search', OSCOM::get_def('error_to_date_less_than_from_date'));
        }
    }
    ...
    if (tep_not_null($pfrom)) {
        if (!settype($pfrom, 'double')) {
            ...
            $message_stack->add_session('search', OSCOM::get_def('error_price_from_must_be_num'));
        }
    }
    if (tep_not_null($pto)) {
        if (!settype($pto, 'double')) {
            $error = true;
            $price_check_error = true;
            $message_stack->add_session('search', OSCOM::get_def('error_price_to_must_be_num'));
        }
    }
    if ($price_check_error == false && is_float($pfrom) && is_float($pto)) {
        if ($pfrom >= $pto) {
            $error = true;
            $message_stack->add_session('search', OSCOM::get_def('error_price_to_less_than_price_from'));
        }
    }
    if (tep_not_null($keywords)) {
        ...
        if (empty($search_keywords)) {
            $error = true;
            $message_stack->add_session('search', OSCOM::get_def('error_invalid_keywords'));
        }
    }
}
if ((!isset($dfrom_date_time) || !$dfrom_date_time->is_valid()) && (!isset($dto_date_time) || !$dto_date_time->is_valid()) && empty($pfrom) && empty($pto) && empty($keywords)) {
    $error = true;
    $message_stack->add_session('search', OSCOM::get_def('error_at_least_one_input'));
}
if ($error == true) {
...
$breadcrumb->add(OSCOM::get_def('navbar_title_2'), OSCOM::link('advanced_search_result.php', tep_get_all_get_params(), true, false));
require $osc_template->get_file('template_top.php');
?>

<div class="page-header">
  <h1><?php 
echo OSCOM::get_def('heading_title_2');
?></h1>
</div>
...
<?php 
// create column list
$define_list = ['PRODUCT_LIST_MODEL' => PRODUCT_LIST_MODEL, 'PRODUCT_LIST_NAME' => PRODUCT_LIST_NAME, 'PRODUCT_LIST_MANUFACTURER' => PRODUCT_LIST_MANUFACTURER, 'PRODUCT_LIST_PRICE' => PRODUCT_LIST_PRICE, 'PRODUCT_LIST_QUANTITY' => PRODUCT_LIST_QUANTITY, 'PRODUCT_LIST_WEIGHT' => PRODUCT_LIST_WEIGHT, 'PRODUCT_LIST_IMAGE' => PRODUCT_LIST_IMAGE, 'PRODUCT_LIST_BUY_NOW' => PRODUCT_LIST_BUY_NOW];
asort($define_list);
$column_list = [];
...
if (isset($dfrom_date_time) && $dfrom_date_time->is_valid()) {
    $search_query .= ' and p.products_date_added >= :products_date_added_from';
}
if (isset($dto_date_time) && $dto_date_time->is_valid()) {
...
if (DISPLAY_PRICE_WITH_TAX == 'true' && (tep_not_null($pfrom) || tep_not_null($pto))) {
    $Qlisting->bind_int(':zone_country_id', $_SESSION['customer_country_id']);
    $Qlisting->bind_int(':zone_id', $_SESSION['customer_zone_id']);
}
$Qlisting->bind_int(':language_id', $OSCOM_Language->get_id());
if (isset($_GET['categories_id']) && tep_not_null($_GET['categories_id'])) {
    $Qlisting->bind_int(':categories_id', $_GET['categories_id']);
    if (isset($_GET['inc_subcat']) && $_GET['inc_subcat'] == '1') {
        for ($i = 0, $n = sizeof($subcategories_array); $i < $n; $i++) {
            $Qlisting->bind_int(':categories_id_' . $i, $subcategories_array[$i]);
        }
    }
}
if (isset($_GET['manufacturers_id']) && tep_not_null($_GET['manufacturers_id'])) {
    $Qlisting->bind_int(':manufacturers_id', $_GET['manufacturers_id']);
}
if (isset($search_keywords) && sizeof($search_keywords) > 0) {
    for ($i = 0, $n = sizeof($search_keywords); $i < $n; $i++) {
        $Qlisting->bind_value(':products_name_' . $i, '%' . $search_keywords[$i] . '%');
        $Qlisting->bind_value(':products_model_' . $i, '%' . $search_keywords[$i] . '%');
        $Qlisting->bind_value(':manufacturers_name_' . $i, '%' . $search_keywords[$i] . '%');
        if (isset($_GET['search_in_description']) && $_GET['search_in_description'] == '1') {
            $Qlisting->bind_value(':products_description_' . $i, '%' . $search_keywords[$i] . '%');
        }
    }
}
if (isset($dfrom_date_time) && $dfrom_date_time->is_valid()) {
    $Qlisting->bind_value(':products_date_added_from', $dfrom_date_time->get_raw(false));
}
if (isset($dto_date_time) && $dto_date_time->is_valid()) {
    $Qlisting->bind_value(':products_date_added_to', $dto_date_time->get_raw(false));
}
if ($pfrom > 0) {
    $Qlisting->bind_decimal(':price_from', $pfrom);
}
if ($pto > 0) {
    $Qlisting->bind_decimal(':price_to', $pto);
}
$Qlisting->set_page_set(isset($_GET['view']) && $_GET['view'] == 'all' ? 999999 : MAX_DISPLAY_SEARCH_RESULTS);
$Qlisting->execute();
...
?>
  ...
  <div class="buttonSet">
    <?php 
echo HTML::button(OSCOM::get_def('image_button_back'), 'fa fa-angle-left', OSCOM::link('advanced_search.php', tep_get_all_get_params(['sort', 'page']), true, false));
?>
  </div>
</div>
