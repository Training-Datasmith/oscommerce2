...
$OSCOM_Language->load_definitions('product_reviews');
...
<?php 
if ($message_stack->size('product_reviews') > 0) {
    echo $message_stack->output('product_reviews');
}
?>
...
<div class="page-header">
  <div class="row">
    <h1 class="col-sm-8" itemprop="name"><?php 
echo $products_name;
?></h1>
    <h2 class="col-sm-4 text-right-not-xs"><?php 
echo $products_price;
?></h2>
  </div>
</div>
...
$Qa->bind_int(':products_id', $Qcheck->value_int('products_id'));
$Qa->bind_int(':languages_id', $OSCOM_Language->get_id());
...
echo OSCOM::get_def('reviews_text_average', ['count' => $Qa->value_int('count'), 'average' => HTML::stars(round($Qa->value('average')))]);
echo '</div>';
?>
...
$Qreviews->bind_int(':products_id', $Qcheck->value_int('products_id'));
...
<div class="row">
  ...
  <div class="col-sm-6">
    <spanclass="pull-right pagenav"><?php 
...
<?php 
    while ($Qreviews->fetch()) {
      ...
      <blockquote class="col-sm-6" itemprop="review" itemscope itemtype="http://schema.org/Review">
        ...
        echo nl2br((string) $Qreviews->value_protected('reviews_text'));
        ?></p>
...
if (isset($_SESSION['navigation']->path[$back])) {
    echo HTML::button(OSCOM::get_def('image_button_back'), 'fa fa-angle-left', OSCOM::link($_SESSION['navigation']->path[$back]['page'], tep_array_to_string($_SESSION['navigation']->path[$back]['get'], ['action'])));
}
?>&nbsp;
    </div>
    <div class="col-xs-6 text-right"><?php 
echo HTML::button(OSCOM::get_def('image_button_write_review'), 'fa fa-commenting', OSCOM::link('product_reviews_write.php', tep_get_all_get_params()));
?></div>
  </div>
</div>
