...
if (OSCOM::has_site_page()) {
    if (OSCOM::is_rpc() === false) {
        ...
        if (empty($page_file) || !is_file($page_file)) {
            $page_file = OSCOM::get_config('dir_root') . 'includes/error_documents/404.php';
        }
...
if (isset($c_path) && tep_not_null($c_path)) {
    ...
    if ($Qcheck->fetch() !== false) {
        ...
        $Qcheck->bind_int(':parent_id', $current_category_id);
        $Qcheck->execute();
        if ($Qcheck->fetch() !== false) {
...
$OSCOM_Language->load_definitions('index');
...
if ($category_depth == 'nested') {
    ...
    $Qcategory->bind_int(':categories_id', $current_category_id);
    $Qcategory->bind_int(':language_id', $OSCOM_Language->get_id());
    $Qcategory->execute();
    ?>

<div class="page-header">
  <h1><?php 
    echo $Qcategory->value('categories_name');
    ?></h1>
</div>
...
<div class="contentContainer">
  <div class="row">
    <?php 
    echo $osc_template->get_content('index_nested');
    ?>
  </div>
</div>
...
<?php 
    ...
    } else if (isset($_GET['filter_id']) && tep_not_null($_GET['filter_id'])) {
    ...
    if (isset($_GET['manufacturers_id']) && !empty($_GET['manufacturers_id'])) {
        if (isset($_GET['filter_id']) && tep_not_null($_GET['filter_id'])) {
            ...
            $Qlisting->bind_int(':categories_id', $_GET['filter_id']);
        } else {
            $Qlisting->bind_int(':language_id', $OSCOM_Language->get_id());
            $Qlisting->bind_int(':manufacturers_id', $_GET['manufacturers_id']);
        }
    } else if (isset($_GET['filter_id']) && tep_not_null($_GET['filter_id'])) {
        $Qlisting->bind_int(':manufacturers_id', $_GET['filter_id']);
        $Qlisting->bind_int(':language_id', $OSCOM_Language->get_id());
        $Qlisting->bind_int(':categories_id', $current_category_id);
    } else {
        $Qlisting->bind_int(':language_id', $OSCOM_Language->get_id());
        $Qlisting->bind_int(':categories_id', $current_category_id);
    }
    ...
    $Qlisting->execute();
    $catname = OSCOM::get_def('heading_title', ['store_name' => STORE_NAME]);
    if (isset($_GET['manufacturers_id']) && !empty($_GET['manufacturers_id'])) {
        ...
        $Qtitle->bind_int(':manufacturers_id', $_GET['manufacturers_id']);
        $Qtitle->bind_int(':languages_id', $OSCOM_Language->get_id());
        $Qtitle->execute();
        ...
        $Qtitle->bind_int(':categories_id', $current_category_id);
        $Qtitle->bind_int(':language_id', $OSCOM_Language->get_id());
        $Qtitle->execute();
        $catname = $Qtitle->value('catname');
    }
    ?>
...
<?php 
    ...
    if (PRODUCT_LIST_FILTER > 0) {
        if (isset($_GET['manufacturers_id']) && !empty($_GET['manufacturers_id'])) {
            ...
            $Qfilter->bind_int(':language_id', $OSCOM_Language->get_id());
            $Qfilter->bind_int(':manufacturers_id', $_GET['manufacturers_id']);
            $Qfilter->execute();
        } else {
            ...
            $Qfilter->bind_int(':categories_id', $current_category_id);
            $Qfilter->execute();
        }
        if ($Qfilter->get_page_set_total_rows() > 1) {
            echo '<div>' . HTML::form('filter', OSCOM::link('index.php', '', false), 'get', null, ['session_id' => true]) . '<p align="right">' . OSCOM::get_def('text_show') . '&nbsp;';
            if (isset($_GET['manufacturers_id']) && !empty($_GET['manufacturers_id'])) {
                echo HTML::hidden_field('manufacturers_id', $_GET['manufacturers_id']);
                $options = [['id' => '', 'text' => OSCOM::get_def('text_all_categories')]];
            } else {
                echo HTML::hidden_field('cPath', $c_path);
                $options = [['id' => '', 'text' => OSCOM::get_def('text_all_manufacturers')]];
            }
            echo HTML::hidden_field('sort', $_GET['sort']);
            while ($Qfilter->fetch()) {
                $options[] = ['id' => $Qfilter->value_int('id'), 'text' => $Qfilter->value('name')];
            }
            echo HTML::select_field('filter_id', $options, $_GET['filter_id'] ?? '', 'onchange="this.form.submit()"');
            echo '</p></form></div>' . "\n";
        }
    }
...
<div class="page-header">
  <h1><?php 
    echo OSCOM::get_def('heading_title', ['store_name' => STORE_NAME]);
    ?></h1>
</div>
...
<div class="row">
  <?php 
    echo $osc_template->get_content('index');
    ?>
</div>
