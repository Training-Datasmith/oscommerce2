...
$OSCOM_Language->load_definitions('checkout_payment_address');
$error = false;
$process = false;
if (isset($_POST['action']) && $_POST['action'] == 'submit' && isset($_POST['formid']) && $_POST['formid'] == $_SESSION['sessiontoken']) {
    ...
    if (tep_not_null($_POST['firstname']) && tep_not_null($_POST['lastname']) && tep_not_null($_POST['street_address'])) {
        ...
        if ($error == false) {
            ...
            $_SESSION['billto'] = $OSCOM_Db->last_insert_id();
        ...
        $Qcheck->bind_int(':address_book_id', $_SESSION['billto']);
...
$breadcrumb->add(OSCOM::get_def('navbar_title_1'), OSCOM::link('checkout_payment.php'));
$breadcrumb->add(OSCOM::get_def('navbar_title_2'), OSCOM::link('checkout_payment_address.php'));
...
<?php 
echo HTML::form('checkout_address', OSCOM::link('checkout_payment_address.php'), 'post', 'class="form-horizontal"', ['tokenize' => true]);
?>
...
if ($process == false) {
  ...
  <h2><?php 
    echo OSCOM::get_def('table_heading_payment_address');
    ?></h2>

  <div class="contentText row">
    <div class="col-sm-8">
      <div class="alert alert-warning"><?php 
    echo OSCOM::get_def('text_selected_payment_destination');
    ?></div>
    </div>
    <div class="col-sm-4">
      <div class="panel panel-primary">
        <div class="panel-heading"><?php 
    echo OSCOM::get_def('title_payment_address');
    ?></div>
...
<?php 
  ...
  <div class="alert alert-info"><?php 
        echo OSCOM::get_def('text_select_other_payment_destination');
        ?></div>
...
<?php 
      ...
      <div class="col-sm-4">
        <div class="panel panel-<?php 
            echo $Qab->value_int('address_book_id') == $_SESSION['billto'] ? 'primary' : 'default';
            ?>">
          <div class="panel-heading"><?php 
          ...
          <div class="panel-footer text-center"><?php 
            echo HTML::radio_field('address', $Qab->value_int('address_book_id'), $Qab->value_int('address_book_id') == $_SESSION['billto']);
            ?></div>
        </div>
      </div>
...
if ($addresses_count < MAX_ADDRESS_BOOK_ENTRIES) {
  ...
  <h2><?php 
    echo OSCOM::get_def('table_heading_new_payment_address');
    ?></h2>

  <div class="alert alert-info"><?php 
    echo OSCOM::get_def('text_create_new_payment_address');
    ?></div>
...
if ($process == true) {
  ...
  <div class="buttonSet">
    <?php 
    echo HTML::button(OSCOM::get_def('image_button_back'), 'fa fa-angle-left', OSCOM::link('checkout_payment_address.php'));
    ?>
  </div>
